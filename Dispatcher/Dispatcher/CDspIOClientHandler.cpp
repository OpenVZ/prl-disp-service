	...
	const SmartPtr<IOPackage>& p )
{
    ...
    case ( PET_IO_CLI_ATTACH_TO_VM ) : {
		Uuid vmUuid;

		// Get vm uuid from attach request
		{
			const PRL_IO_ATTACH_REQUEST* attachRequest =
				reinterpret_cast<const PRL_IO_ATTACH_REQUEST*>( p->buffers[0].getImpl() );

			if ( p->data[0].bufferSize <
				 offsetof(PRL_IO_ATTACH_REQUEST, vmUuid) + sizeof(attachRequest->vmUuid) ) {
				WRITE_TRACE(DBG_FATAL, "Wrong attach to vm package!");
				CDspService::instance()->getIOServer().disconnectClient( h );
				break;
			}

			vmUuid = Uuid::toUuid( attachRequest->vmUuid );
			if ( vmUuid.isNull() ) {
				WRITE_TRACE(DBG_FATAL, "Wrong attach to vm package!");
				CDspService::instance()->getIOServer().disconnectClient( h );
				break;
		...
		if ( ! m_ioClients.contains(h) || m_ioClients[h].sessionId.isEmpty() ) {
			WRITE_TRACE(DBG_FATAL, "Client '%s' is not authed! Close connection!", QSTR2UTF8(h));
			CDspService::instance()->getIOServer().disconnectClient( h );
			return;
		}
		...
		// Get client
		SmartPtr<CDspClient> client = CDspService::instance()->
			getClientManager().getUserSession( m_ioClients[h].sessionId );
		if ( ! client.isValid() ) {
			WRITE_TRACE(DBG_FATAL, "Can't find client by session Id! "
				"Close connection!");
			CDspService::instance()->getIOServer().disconnectClient( h );
			return;
		}

		m_ioClients[h].vmIdent = client->getVmIdent( vmUuid );

		locker.unlock();
		// unlock to prevent deadlocks in check encryption block
		//////////////////////////////////////////////////////////////////////////


		// Find Vm by vm uuid
		SmartPtr<CDspVm> vm =
			CDspVm::GetVmInstanceByUuid( client->getVmIdent(vmUuid) );

		// Check vm
		if ( ! vm.isValid() ) {
			WRITE_TRACE(DBG_FATAL, "Vm '%s' was not found!",
						qPrintable(vmUuid.toString()));
			sendVmAttachResponse( h, false, false );
			// Close connection
			CDspService::instance()->getIOServer().disconnectClient( h );
			return;
		}
		...
		PRL_ASSERT( m_ioClients.contains(h) );
		PRL_ASSERT( !m_ioClients[h].sessionId.isEmpty() );
		if ( ! m_ioClients.contains(h) || m_ioClients[h].sessionId.isEmpty() ) {
			CDspService::instance()->getIOServer().disconnectClient( h );
			return;
		}
		...
		// Get client info
		ClientInfo& clientInfo = m_ioClients[h];


		// Save Vm handle
		clientInfo.vmHandle = vm->getVmConnectionHandle();

		// Get pid
		quint32 pid = vm->getVmProcessIdAsUint();
		...
		// Do Detach
		bool res =
			CDspService::instance()->getIOServer().detachClient( h, pid, p );
		if ( ! res ) {
			WRITE_TRACE(DBG_FATAL, "Can't detach client!");
			sendVmAttachResponse( h, false, false );
			// Close connection
			CDspService::instance()->getIOServer().disconnectClient( h );
			return;
