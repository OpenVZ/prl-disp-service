...
QString CDspVmSnapshotStoreHelper::getSnapshotTreeWithBase64Images( SmartPtr<CDspClient> pUser
	...
	)
{
	...
	bool bScreenshotsInFullSize = nFlags & PGST_FULL_SIZE_SCREENSHOTS;
	...
	//////////////////////////////////////////////////////////////////////////
	// get plain list of states
	int i, j;
	QList<CSavedStateTree *> snapList;
	CSavedStateTree *pSnap;

	if ((pSnap = snapTree.GetSavedStateTree()) == NULL)
		return "";
	snapList.append(pSnap);
	for (i = 0; i < snapList.size(); ++i)
	{
		for (j = 0; j < snapList.at(i)->GetChildCount(); ++j)
			snapList.append( snapList.at(i)->GetChild(j) );
	}

	//////////////////////////////////////////////////////////////////////////
	// get vm folder
	QString sVmHome = CDspVmDirManager::getVmHomeByUuid( pUser->getVmIdent( sVmUuid ) );
	if( sVmHome.isEmpty() )
		return "";
	QString sVmFolder = QFileInfo(sVmHome).absolutePath();

	//////////////////////////////////////////////////////////////////////////
	// decrypt encrypted snapshots and add they as base64
	bool bTraceWarning = true;
	foreach( CSavedStateTree* pState, snapList )
	{
		PRL_ASSERT(pState);

		QByteArray baStateBuffer;
		if( pState->GetScreenShot().isEmpty() )
			continue;

		// cleanup screenshot content to prevent send huge base64 which hangs gui.
		QString screenshotContent = pState->GetScreenShot();
		pState->SetScreenShot("");

		if( nFlags & PGST_WITHOUT_SCREENSHOTS )
		{
			if( bTraceWarning)
			{
				WRITE_TRACE( DBG_INFO, "All snapshots will be skipped in response by user request." );
				bTraceWarning = false;
			}
		...
		QString fileToLoad = QString( "%1/%2/%3" ).arg(sVmFolder)
			.arg(VM_GENERATED_WINDOWS_SNAPSHOTS_DIR).arg(screenshotContent);

		QTemporaryFile tmpFile( sVmFolder + "/" + "SnapTreeTmp.XXXXXXXXX" );
		if( !QFile::exists( fileToLoad ) ) // try to load string content as Base64
		{
			//////////////////////////////////////////////////////////////////////////
			// copy encrypted content to file(  to decrypt )
			if( !tmpFile.open() )
			{
				WRITE_TRACE( DBG_FATAL, "Can't open tmp file %s", QSTR2UTF8(tmpFile.fileName()) );
				continue;
			}
			fileToLoad = tmpFile.fileName(); // check that it is absolute

			QByteArray ba = QByteArray::fromBase64( screenshotContent.toUtf8() );
			qint64 res = tmpFile.write( ba );
			if( res<=-1 || res != ba.size() )
			{
				WRITE_TRACE( DBG_FATAL, "Can't write to tmp file %s by error %lld"
					, QSTR2UTF8(tmpFile.fileName()), res );
				continue;
			}
			if( !HostUtils::FlushQFile(tmpFile)
#ifdef _WIN_
				// to prevent assert() detected by sandro@
				|| tmpFile.handle()< 0 || _commit( tmpFile.handle() )
#endif
				)
			{
				WRITE_TRACE( DBG_FATAL, "Can't flush tmp file %s"
					, QSTR2UTF8(tmpFile.fileName()));
				continue;
			}
			tmpFile.close();
		}
		...
		QByteArray ba;
		{
			...
			{
				WRITE_TRACE(DBG_WARNING, "Can't load shot %s for vmuuid = %s"
					, QSTR2UTF8( pState->GetGuid() )
					, QSTR2UTF8( sVmUuid) );
				continue;
			}
			ba = f.readAll();
		...
		if( !bScreenshotsInFullSize )
		{
			QImage img;
			if( !img.loadFromData(ba, "PNG" ))
			{
				WRITE_TRACE( DBG_INFO, "Can't load png image." );
				continue;
			}

			QImage scaledImg = img.scaled( DEFAULT_SCREENSHOT_WIDTH_FOR_SNAPSHOT
				, DEFAULT_SCREENSHOT_HEIGHT_FOR_SNAPSHOT
				, Qt::IgnoreAspectRatio
				, Qt::SmoothTransformation );

			QBuffer scaledBuff(&ba);
			if( !scaledImg.save( &scaledBuff, "PNG" ) )
			{
				WRITE_TRACE( DBG_INFO, "Can't save scaled png image." );
				continue;
			}
		}

		pState->SetScreenShot( QString::fromAscii( ba.toBase64() ));
	}//foreach
	snapTree.Save( buffer, true );

#if 0
	PUT_RAW_MESSAGE( "!!!!!!!!!!BUFFER:" );
	PUT_RAW_MESSAGE( buffer.data() );
	PUT_RAW_MESSAGE( "\n" );
#endif

	return UTF8_2QSTR( buffer.data() );
}
