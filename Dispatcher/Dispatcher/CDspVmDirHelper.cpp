...
//
void CDspVmDirHelper::validateSectionVmConfig(SmartPtr<CDspClient> pUserSession, const SmartPtr<IOPackage>& pkg)
{
	// retrieve user parameters from request data
	...
	CVmValidateConfig vmValidateConfig(pVmConfig);
	QList< PRL_RESULT > lstResults = vmValidateConfig.CheckVmConfig( nSection, pUserSession );
...
PRL_RESULT CDspVmDirHelper::insertVmDirectoryItem(const QString &sDirUuid, CVmDirectoryItem* vm_item)
{
	...
	// #9246
	if (vm_item->getVmType() == PVT_VM)
	{
		removeSecureData( vm_item->getVmUuid() );
	}
	...
	)
{
	...
	if ( bFillAutogenerated ) {
		foreach ( CVmGenericNetworkAdapter* device, pOutVmConfig->getVmHardwareList()->m_lstNetworkAdapters ) {
			device->setHostInterfaceName(PrlNet::getDefaultVirtAdapterName(device, pOutVmConfig));
		}
	}
...
PRL_RESULT CDspVmDirHelper::getVmStartUser( SmartPtr<CVmConfiguration> vmConfig
						 , SmartPtr<CDspClient> &pRunUser
						, bool bAutoStart)
{
	PRL_RESULT ret = PRL_ERR_VM_USER_AUTHENTICATION_FAILED;

	PRL_ASSERT( vmConfig );

	PRL_VM_START_LOGIN_MODE mode = vmConfig->getVmSettings()->
					getVmStartupOptions()->getVmStartLoginMode();
	bool bFastReboot = vmConfig->getVmSettings()->getVmStartupOptions()->isFastReboot();
	if (bFastReboot)
	{
		QString user;

		ret = loadFastRebootData( vmConfig->getVmIdentification()->getVmUuid(), user );
		if ( PRL_SUCCEEDED(ret) && !user.isEmpty() )
		{
			pRunUser = SmartPtr<CDspClient>(new CDspClient( IOSender::Handle(), user ));
			return PRL_ERR_SUCCESS;
		}
		else
		{
			if ( user.isEmpty() )
				WRITE_TRACE(DBG_FATAL, "FastReboot: Specified empty fast reboot user");
			else
				WRITE_TRACE(DBG_FATAL, "FastReboot: Failed to load user.");
		}

	}

	if ( mode == PLM_START_ACCOUNT )
	{
		if ( bAutoStart )
		{
			PRL_ASSERT( ! vmConfig->getVmIdentification()->getHomePath().isEmpty() );

			QString qsOwner = CAuthHelper::GetOwnerOfFile(vmConfig->getVmIdentification()->getHomePath());
			if (qsOwner.isEmpty())
				return ret;

			pRunUser = SmartPtr<CDspClient>(new CDspClient( IOSender::Handle(), qsOwner ));
#ifdef _WIN_
			WRITE_TRACE( DBG_FATAL, "Cannot start VM by owner user, need password for authorization !" );
#endif
		}

		ret = PRL_ERR_SUCCESS;
	}
	else if ( mode == PLM_ROOT_ACCOUNT )
	{
		WRITE_TRACE(DBG_FATAL, "PLM_ROOT_ACCOUNT");
		pRunUser = SmartPtr<CDspClient>(new CDspClient( IOSender::Handle() ));
		if ( pRunUser->getAuthHelper().AuthUserBySelfProcessOwner())
			ret = PRL_ERR_SUCCESS;
	}
	else if ( mode == PLM_USER_ACCOUNT )
	{
...
PRL_RESULT CDspVmDirHelper::saveSecureData( SmartPtr<CDspClient> /*pUserSession*/
	, SmartPtr<CVmConfiguration> /*vmConfig*/, CVmEvent& /*errorEvent*/ )
{
	return PRL_ERR_UNIMPLEMENTED;
}

bool CDspVmDirHelper::loadSecureData( SmartPtr<CVmConfiguration> /*vmConfig*/ )
{
	// not implemented
	return false;
}

void CDspVmDirHelper::resetSecureParamsFromVmConfig(SmartPtr<CVmConfiguration> vmConfig)
{
	LOG_MESSAGE( DBG_DEBUG, "resetSecureParamsFromVmConfig");
	vmConfig->getVmSettings()->getVmStartupOptions()->setVmStartAsPassword();
	vmConfig->getVmSettings()->getVmStartupOptions()->setChangedPassword();
}

void CDspVmDirHelper::removeSecureData(const QString& vm_uuid)
{
	QString serverUuid = CDspService::instance()->getDispConfigGuard().getDispConfig()-> \
		getVmServerIdentification()->getServerUuid();

	CDspLockedPointer<QSettings> pSettings = CDspService::instance()->getQSettings();
	QString key = QString( "%1/%2" ).arg( serverUuid ).arg( vm_uuid );

	LOG_MESSAGE( DBG_DEBUG, "removeSecureData: %s", QSTR2UTF8(key) );

	SafeDeleteKeyInQSetting( pSettings, QString( "%1/%2" ).arg( key ).arg( SECURE_TAG_USERCRED ) );
	SafeDeleteKeyInQSetting( pSettings, QString( "%1/%2" ).arg( key ).arg( SECURE_TAG_USERPASS ) );
	SafeDeleteKeyInQSetting( pSettings, QString( "%1/%2" ).arg( key ).arg( SECURE_TAG_AUTOSTART ) );

	pSettings->sync();
	LOG_MESSAGE( DBG_DEBUG, "removeSecureData4 sync: %s", QSTR2UTF8(key) );
}
