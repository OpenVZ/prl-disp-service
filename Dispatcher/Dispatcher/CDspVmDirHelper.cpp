		...
		bool bAbsolute, bool bLoadConfigDirectlyFromDisk )
{
	...
	else
	{
		...
		pVmConfig->getVmIdentification()->setCtId(
				QString::number(Uuid::toVzid(pDirectoryItem->getVmUuid())));
	}
	...
	)
{
	...
	// LOCK inside brackets
	{
		...
		if (pOutVmConfig->getVmSettings()->getLockDown())
	...
	if (pOutVmConfig->getVmSettings()->getVmRemoteDisplay() &&
		pOutVmConfig->getVmSettings()->getVmRemoteDisplay()->getMode() == PRD_DISABLED)
...
void CDspVmDirHelper::resetAdvancedParamsFromVmConfig( SmartPtr<CVmConfiguration> pOutVmConfig )
{
	...
	case PRD_DISABLED:
		...
		break;
	case PRD_AUTO:
		if (remDisplay->getHostName() != QHostAddress(QHostAddress::LocalHost).toString())
		{
...
Filter::result_type& Filter::do_(result_type &output)
{
	const value_type& r = output.value();

	QList<CVmGenericNetworkAdapter* > na;
	foreach (CVmGenericNetworkAdapter* a, r->getVmHardwareList()->m_lstNetworkAdapters)
	{
		if (m_predicate.empty() || !m_predicate(a))
...
Filter::result_type Filter::handle(const CVmDirectoryItem& item_)
{
	result_type output = Chain::handle(item_);
	if (output.isFailed())
		return output;
...
Chain* Factory::operator()(const session_type& session_, quint32 flags_) const
{
	...
	if (flags_ & PGVLF_GET_NET_STATIC_IP_INFO)
	{
		Filter* f = new Filter();
		f->setPredicate(boost::bind(&QList<QString>::isEmpty,
			boost::bind(&CVmGenericNetworkAdapter::getNetAddresses, _1)));
		if (output == NULL)
			output = f;
		else
			tail->setNext(f);
	...
	if (flags_ & PGVLF_GET_ONLY_IDENTITY_INFO)
		...
		tail->setNext(new Sterling(session_, *m_service));
