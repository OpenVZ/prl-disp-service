...
namespace
{
bool readVmToolsStateString(QString &toolsVersionStr, const CVmIdent &vmIdent)
{
	PRL_RESULT nRetCode = PRL_ERR_UNINITIALIZED;
	SmartPtr<CVmConfiguration> pVmConfig =
		DspVm::vdh().getVmConfigByUuid(vmIdent.second, vmIdent.first, nRetCode);

	if (0 == pVmConfig)
	{
		WRITE_TRACE(DBG_FATAL, "Couldn't to find VM configuration "\
			"for '%s' VM UUID which belongs to '%s' VM dir",\
			qPrintable(vmIdent.first), qPrintable(vmIdent.second));
		return false;
	}
...
Details::Details(const CVmIdent& id_, const SmartPtr<CDspClient>& client_,
	...
	m_sVmToolsState(getVmToolsStateStringOffline(id_)),
...
QString Details::getVmToolsStateStringOffline(const CVmIdent &vmIdent)
{
	QString toolsVersionStr;

	const bool isInstalled = readVmToolsStateString(toolsVersionStr, vmIdent);

	const unsigned toolsState = (isInstalled) ? PTS_POSSIBLY_INSTALLED
		: PTS_NOT_INSTALLED;

	CVmEvent evt;
	evt.addEventParameter( new CVmEventParameter(PVE::Integer
		, QString("%1").arg((int)toolsState)
		, EVT_PARAM_VM_TOOLS_STATE ));

	evt.addEventParameter( new CVmEventParameter(PVE::String
		, QString("%1").arg(toolsVersionStr)
...
Routine::Routine(const CVmIdent& ident_): m_left(HANDSHAKE_TIMEOUT), m_ident(ident_)
{
}

void Routine::reply()
{
	SmartPtr<IOPackage> p = m_vm->getStartVmRequestPackage();
	if (!p.isValid())
		return;

	SmartPtr<CDspClient> u = m_vm->getVmRunner();
	if (!u.isValid())
		return;
	PRL_RESULT e = PRL_ERR_VM_PROCESS_IS_NOT_STARTED;
	//We have valid request info - let send response to requester
	if (m_vm->getVmProcessId())
	{
		// https://bugzilla.sw.ru/show_bug.cgi?id=132309
		// [ Unable to start the virtual machine due to software incompatibility. ]
#ifdef _WIN_
		PROCESS_INFORMATION* pPI = m_vm->getVmProcessId();
		DWORD x = 0;
		if( GetExitCodeProcess(pPI->hProcess, &x ) && x == PRL_VM_CANT_CONNECT_TO_DISPATHER_RETCODE)
			e = PRL_ERR_VM_PROCESS_IS_NOT_STARTED_BY_WRONG_HOST_SW;
#else
		// NOTE:
		//	Now we fixed in only on WINDOWS platform because we havent any same problems on UNIX.
		//	But if we have we can implement same mech on linux and mac too:

		// On LINUX through waitid( WNOHANG | WNOWAIT ) syscall
		//	( WNOWAIT - Leave the child in a waitable state;
		//		a later wait call can be used to again retrieve the child status information.)
		// On MAC through kevent() mech.
#endif
	}
	WRITE_TRACE(DBG_FATAL, "CDspVmProcessMonitor: send error %#x %s",
			e, PRL_RESULT_TO_STRING(e));
	u->sendSimpleResponse(p, e);
}
...
void Routine::fail()
{
	...
	switch (m_vm->getVmState())
	{
	case VMS_RECONNECTING:
		return CDspVm::UnregisterVmObject(m_vm);
	case VMS_SUSPENDED:
	case VMS_SUSPENDING_SYNC:
		CDspVm::UnregisterVmObject(m_vm);
		return reply();
	default:
		if (m_vm->isUndoDisksMode())
			break;
	case VMS_SUSPENDING:
	{
		CVmIdent i = m_vm->getVmIdent();
		CDspVm::UnregisterVmObject(m_vm);
		CVmEvent e(PET_DSP_EVT_VM_STOPPED, i.first, PIE_VIRTUAL_MACHINE);
		SmartPtr<IOPackage> p = DispatcherPackage::createInstance(PVE::DspVmEvent, e.toString());
		// send stop event to clients!
		cm().sendPackageToVmClients(p, i.second, i.first);
		return reply();
	}
	}
	if (!m_vm->startUndoDisksRevertOrCommitTask())
		CDspVm::UnregisterVmObject(m_vm);
}
...
bool Routine::wait()
{
	...
	quint32 p = m_vm->getVmProcessIdAsUint();
	// VM still in starting state - let check whether VM process alive
	// the vm process wasn't created yet
	if (0 != p && !checkWhetherProcessAlive(p))
		return false;

	HostUtils::Sleep(MONITORING_TIMEOUT);
	if (m_vm->handshakeWasCalled())
		return true;

	m_left -= MONITORING_TIMEOUT;
	if (0 < m_left)
		return true;

	WRITE_TRACE(DBG_FATAL, "Handshake timeout( %d sec )  elapsed for vm %s %s",
		HANDSHAKE_TIMEOUT/1000, QSTR2UTF8(m_vm->getVmName()),
		QSTR2UTF8(m_vm->getVmUuid()));
	return false;
}

bool Routine::isPending()
{
	m_vm = CDspVm::GetVmInstanceByUuid(m_ident.first, m_ident.second);
	if (!m_vm.isValid())
	{
		//Seems VM process already finalized
		return false;
	}
	switch (m_vm->getVmState())
	{
	...
	case VMS_RECONNECTING:
		return true;
	default:
		// VM already handshaked with dispatcher so finalize monitoring now
		return false;
	}
}

SmartPtr<CDspVm> Routine::celebrate()
{
	if (!m_vm.isValid())
		return SmartPtr<CDspVm>();

	QString v = m_vm->getRestoredVmProductVersion();
	if (v.isEmpty() || v == VER_FULL_BUILD_NUMBER_STR)
		return m_vm;

	WRITE_TRACE(DBG_FATAL,
		"Mismatch versions between Vm uuid=%s (%s) and dispatcher (%s) !",
		QSTR2UTF8(m_ident.first), QSTR2UTF8(v), VER_FULL_BUILD_NUMBER_STR);

	if (CMainDspService::allowSkipVersionCheckOnVmReconnect())
		return m_vm;

	WRITE_TRACE(DBG_FATAL, "Restored VM uuid=%s will be stopped ! ",
		QSTR2UTF8(m_ident.first));
	m_vm->changeVmState(VMS_STOPPING);

	CProtoCommandPtr pCmd = CProtoSerializer::CreateProtoVmCommandStop(m_ident.first, PSM_KILL, PSF_FORCE);
	SmartPtr<IOPackage> p = DispatcherPackage::createInstance(PVE::DspCmdVmStop, pCmd);
	m_vm->sendPackageToVm(p);
	return SmartPtr<CDspVm>();
}
...
SmartPtr<CDspVm> Standard::operator()() const
{
	Routine u(m_ident);
	while (u.isPending())
	{
...
void Extended::operator()() const
{
	SmartPtr<CDspVm> v = m_chain();
	if (v.isValid() && v->getVmState() != VMS_MIGRATING)
		Task_CommitUnfinishedDiskOp::pushVmCommit(*v);
}
