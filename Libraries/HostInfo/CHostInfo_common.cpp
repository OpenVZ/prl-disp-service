...
//static QString make_sound_user_name(PaDeviceIndex id, bool is_default);
//static int get_default_sound_id(bool is_input);
...
//	if (nFlags & uhiSound)
//	{
//		// clear list of input sound
//		CleanupGenericDevicesList(p_HostHwInfo->m_lstSoundMixerDevices,
//									p_HostHwInfo->getSoundDevices()->getMixerDevices());
//		// clear list of output sound
//		CleanupGenericDevicesList(p_HostHwInfo->m_lstSoundOutputDevices,
//									p_HostHwInfo->getSoundDevices()->getOutputDevices());
//		// get list of sound output devices
//		GetSoundLists();
//		WRITE_TRACE(DBG_DEBUG, "--- sound info refreshed ---");
//	}
...
///**
// * Convert partitions map to harware info class
// *
// * @param HDD partitions list
// * @param Partitions map
// *
// * @author antonz@
// *
// * @return Error code
// */
//PRL_RESULT CDspHostInfo::ConvertToHardwareInfo(QList<CHwHddPartition*>& pHddParts,
//											 const CPartition::InfoMap& Parts,
//											 const QString& hddName)
//{
//	// Define types
//	typedef std::map<PRL_UINT32, QList<CHwHddPartition*> > PartitionsMap;
//	typedef PartitionsMap::iterator PartitionsMapIter;
//	typedef QList<CHwHddPartition*>::iterator HwPartitionIter;
//	typedef std::pair<HwPartitionIter, HwPartitionIter> HwPartitionStartEndIter;
//
//	// Paritions iterator
//	CPartition::InfoMap::const_iterator itIMStart, itIMEnd;
//	// All partitions map
//	PartitionsMap mapParts;
//	// Iterators
//	PartitionsMapIter itPMap, itPMapEnd, itPMapTmp;
//	// Temporary list to create
//	QList<CHwHddPartition*> TmpList;
//	// Temporary CHwHddPartition pointer
//	CHwHddPartition* pPartition;
//	// Last tree building stack
//	std::stack<HwPartitionStartEndIter> treeStack;
//
//	// Move all parts to the map according to parent ID
//	for(itIMStart = Parts.begin(), itIMEnd = Parts.end();
//		itIMStart != itIMEnd;
//		itIMStart++)
//	{
//		// Partition system name
//		QString systemName = hddName;
//		// Partition ID
//		PRL_UINT32 uiPID = itIMStart->first;
//
//		itPMap = mapParts.find(itIMStart->second.uParentID);
//		// Create partition and set up parameters
//		pPartition = new CHwHddPartition;
//
//		// Special workaround for GPT partitions
//		if (itIMStart->second.uParentID == GPT_PARTITION_ID)
//			uiPID -= GPT_PARTITION_ID;
//
//		// Generate system name
//#ifdef _WIN_
//		systemName += QString(":%1").arg(uiPID);
//#elif defined _LIN_
//		if (systemName.startsWith(UDEV_HDD_BASE))
//			systemName += QString("-part%1").arg(uiPID);
//		else
//			systemName += QString("%1").arg(uiPID);
//#elif defined _MAC_
//		QString partName;
//		QString partId = itIMStart->second.Id.toStringWithoutBrackets();
//		// make uuid string os compatible
//		partId = partId.toUpper();
//
//		QString diskName = hddName.section(QDir::separator(), -1);
//		systemName += QString("s%1").arg(uiPID);
//
//		if (PRL_SUCCEEDED(IMacHelpers::GetNameByUID(partId, partName)) &&
//			partName.startsWith(diskName))
//		{
//			systemName = hddName.section(QDir::separator(), 0, -2) +
//									QDir::separator() + partName;
//		}
//#endif
//		pPartition->setSystemName(systemName);
//		// +1 sector?
//		pPartition->setSize((itIMStart->second.uiEnd - itIMStart->second.uiStart) * SECTOR_SIZE);
//		// Store partition ID
//		pPartition->setIndex(itIMStart->first);
//
//		if (CPartition::IsGPTPartition(itIMStart->second))
//		{
//			pPartition->setType(CGPTConvertor::GetMbrTypeByGUID(itIMStart->second.Guid));
//			pPartition->setName(itIMStart->second.Name);
//
//			LOG_MESSAGE(DBG_DEBUG, "Added GPT partition type %u name %s",
//					pPartition->getType(), pPartition->getName().toUtf8().data());
//		}
//		else
//		{
//			pPartition->setType(itIMStart->second.uiType);
//			// FIXME: Generate name by type of partition
//			pPartition->setName(QString(""));
//
//			LOG_MESSAGE(DBG_DEBUG, "Added MBR partition type %u name %s",
//					pPartition->getType(), pPartition->getName().toUtf8().data());
//		}
//
//		pPartition->setIsActive(itIMStart->second.bBootable);
//
//		if (itPMap == mapParts.end())
//		{
//			// Clear list
//			TmpList.clear();
//			// Create new partition and add to the list
//			TmpList.push_back(pPartition);
//
//			mapParts.insert(std::pair<PRL_UINT32, QList<CHwHddPartition*> >
//							 (itIMStart->second.uParentID, TmpList)
//							);
//		} else {
//			// Add to existing list
//			itPMap->second.push_back(pPartition);
//		}
//	}
//
//	// Search for the base partition
//	itPMapTmp = mapParts.find(MBR_PARTITION_ID);
//	if (itPMapTmp != mapParts.end())
//	{
//		pHddParts += itPMapTmp->second;
//		mapParts.erase(itPMapTmp);
//	}
//
//	// Add there GUID partitions
//	itPMapTmp = mapParts.find(GPT_PARTITION_ID);
//	if (itPMapTmp != mapParts.end())
//	{
//		pHddParts += itPMapTmp->second;
//		mapParts.erase(itPMapTmp);
...
//
//	/*
//	 * Ok, now we should cycle through found partitions
//	 * and link lists
//	 */
//	treeStack.push(HwPartitionStartEndIter(pHddParts.begin(), pHddParts.end()));
//
//	// Crosslink structures
//	while (treeStack.size())
//	{
//		HwPartitionStartEndIter itCurrent = treeStack.top();
//		HwPartitionStartEndIter itTmp;
//		HwPartitionIter itHwP = itCurrent.first;
//
//		// Remove element from stack
//		treeStack.pop();
//
//		// Run through all elements
//		while(itHwP != itCurrent.second)
//		{
//			// Cache parition pointer
//			pPartition = *itHwP;
//			// Jump to the next iterator
//			itHwP++;
//
//			itPMapTmp = mapParts.find(pPartition->getIndex());
//			// Next iteration
//			if (itPMapTmp != mapParts.end())
//			{
//				// Found related partition, link it
//				pPartition->m_lstPartitions += itPMapTmp->second;
//				mapParts.erase(itPMapTmp);
//				itTmp = HwPartitionStartEndIter(pPartition->m_lstPartitions.begin(),
//												pPartition->m_lstPartitions.end());
//
//				// Store back
//				itCurrent.first = itHwP;
//				treeStack.push(itCurrent);
//				// Add next element
//				treeStack.push(itTmp);
//				// Break inner cycle
//				break;
//			}
//		}
//		// Go to the next stack iteration
//	}
//
...
// DiskImage commented out by request from CP team
//	// Add hard disk
//	IDiskImage* pImage = IDiskImage::OpenImage(systemName,
//												PRL_DISK_READ,
//												PRL_IMAGE_PHYSICAL,
//												&Err, NULL);
//
//	if (!pImage)
//	{
//		WRITE_TRACE(DBG_FATAL, "Error opening disk %s code 0x%x",
//					QSTR2UTF8(systemName), Err);
//		return false;
//	}
//
//	// Partition list (internal representation)
//	CPartition::InfoMap Partitions;
//
//	if (!uiSize)
//	{
//		// Get disk parameters
//		IDiskImage::Parameters Params;
//		pImage->GetParameters(Params);
//		uiDiskSize = Params.Size * SECTOR_SIZE;
//	}
//
//	// Get partitions list
//	Err = CPartition::EnumeratePartitions(pImage, Partitions);
//
...
// DiskImage commented out by request from CP team
//	// Convert detected partitions to HwList
//	ConvertToHardwareInfo(pHwDisk->m_lstPartitions, Partitions, systemName);
	// Add disk to hwinfo
	List.append( pHwDisk );
...
// PortAudioWrap commented out by request from CP team
//// construct system name for sound device (PortAudio)
//static QString make_sound_sys_name(PaDeviceIndex id, bool is_default)
//{
//	// system name format: %1|%2
//	//   %1 -- PA device id
//	//   %2 -- default device flag
//	//   %3 -- user friendly name hash (SHA1)
//
//	QByteArray hash = QCryptographicHash::hash(make_sound_user_name(id, is_default).toUtf8(),
...
bool CDspHostInfo::ParseSoundSystemName(bool /*is_input*/, const QString& /*sys_name*/,
								int* /*id*/, bool* /*is_default*/, bool* /*is_null*/)
{
// PortAudioWrap commented out by request from CP team
//	int		real_id;
//	bool	real_is_default;
//
//	QStringList parts;
//
//	PaError pa_err;
...
//	bool no_such_device;
//
//	// special case: empty string (old format compatibility)
//	if (sys_name.isEmpty()) {
//		real_id = paNoDevice;
//		real_is_default = true;
//		goto check_default;
//	}
//
...
//
//	bool is_valid;
//	real_id = parts.at(0).toInt(&is_valid);
//	if (!is_valid)
//		return false;
//
//	real_is_default = parts.at(1).toInt(&is_valid);
//	if (!is_valid)
//		return false;
//
//	// fast check for avoid heavy operations
//	if (id == NULL && is_null == NULL)
//		goto fini;
//
//	// NOTE: this is kludge for early development PD6 system name
//	// if system name don't have hash, fallback to default to avoid errors
//	if (parts.count() < 3 && real_id != paNoDevice) {
//		real_id = paNoDevice;
//		real_is_default = true;
//	}
//
//check_default:
//
//	// is this "Default" device ?
//	if (real_is_default) {
//		real_id = get_default_sound_id(is_input);
//		goto fini;
//	}
//
//	// is this "Null" device ?
//	if (real_id == paNoDevice)
//		goto fini;
//
//	//
//	// try to get real device id
//	//
//
//	// initialize PA here to get fresh device list
//	pa_err = Pa_InitializeSafe();
//	if (pa_err != paNoError) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] Pa_InitializeSafe() failed: %d (%s)",
//			pa_err, Pa_GetErrorText(pa_err));
//
//		// use id from dispatcher
//		goto fini;
//	}
//
//	no_such_device = true;
//	for (PaDeviceIndex dev_id = 0; dev_id < Pa_GetDeviceCount(); dev_id++) {
//
//		const PaDeviceInfo*	dev_info = Pa_GetDeviceInfo(dev_id);
//		if (dev_info == NULL) {
//			WRITE_TRACE(DBG_FATAL,
//				"[HostInfo] ParseSoundSystemName(), Pa_GetDeviceInfo(%d) failed",
//				dev_id);
//			continue;
//		}
//
//		// skip playback only device if we need capture
//		if (is_input && dev_info->maxInputChannels == 0)
//			continue;
//		// skip capture only device if we need playback
//		if (!is_input && dev_info->maxOutputChannels == 0)
//			continue;
//
//		QByteArray hash = QCryptographicHash::hash(make_sound_user_name(dev_id, false).toUtf8(),
//													QCryptographicHash::Sha1);
//		if (parts.at(2) == QString(hash.toHex())) {
//			if (real_id != dev_id) {
//				WRITE_TRACE(DBG_DEBUG,
//					"[HostInfo] Real sound device ID is %d instead %d",
//					dev_id, real_id);
//			}
//			real_id = dev_id;
...
//	}
//
//	// shutdown this PA instance
//	pa_err = Pa_TerminateSafe();
//	if (pa_err != paNoError) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] Pa_TerminateSafe() failed: %d (%s)",
//			pa_err, Pa_GetErrorText(pa_err));
//	}
//
//	// specified device not found, return failure to throw connect error
//	if (no_such_device)
//		return false;
//
//fini:
...
//		*id = real_id;
//	if (is_default != NULL)
//		*is_default = real_is_default;
//	if (is_null != NULL)
//		*is_null = (real_id == paNoDevice);
//
//	return true;
	return false;
}
...
// PortAudioWrap commented out by request from CP team
//// construct user friendly name for sound Device
//static QString make_sound_user_name(PaDeviceIndex id, bool is_default)
//{
//	// special case: "default" and "null" device
//	if (id == paNoDevice) {
//		return is_default ?	PRL_SOUND_DEFAULT_DEVICE_NAME :
//							PRL_SOUND_NULL_DEVICE_NAME;
//	}
//
//	PaError pa_err;
//	QString name(QString("[?] %1").arg(id));
//
//	PaHostApiIndex dev_api_count = 0;
//	const PaHostApiInfo *dev_api_info = NULL;
//
//	// initialize PA before touch Pa_GetDeviceInfo()
//	pa_err = Pa_InitializeSafe();
//	if (pa_err != paNoError) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] make_sound_user_name(), Pa_InitializeSafe() failed: %d (%s)",
//			pa_err, Pa_GetErrorText(pa_err));
//
//		return name;
//	}
...
//	if (dev_info == NULL) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] make_sound_user_name(), Pa_GetDeviceInfo(%d) failed",
//			id);
//
//		goto fini;
//	}
//
//	name = QString::fromLocal8Bit(dev_info->name);
//
//	dev_api_count = Pa_GetHostApiCount();
//	if (dev_api_count < 0) {
//		WRITE_TRACE(DBG_FATAL,
//				"[HostInfo] make_sound_user_name(), Pa_GetHostApiCount() failed: %s",
//				Pa_GetErrorText(dev_api_count));
//
//		name.prepend("[?] ");
//		goto fini;
//	}
//
//	//
//	// check if host have only one API, in that case host API prefix is
//	// not needed
//	// TODO: specific host API can handle zero devices, by the way we
//	//  should check for one _active_ API condition here
//	//
//	if (dev_api_count < 2)
//		goto fini;
//
//	//
//	// get host API name and return device name + host API name
//	//
//	dev_api_info = Pa_GetHostApiInfo(dev_info->hostApi);
//	if (dev_api_info == NULL) {
//		WRITE_TRACE(DBG_FATAL,
//				"[HostInfo] make_sound_user_name(), Pa_GetHostApiInfo(%d) failed",
//				id);
//
//		name.prepend("[?] ");
//		goto fini;
...
//
//	//
//	// NOTE: some host API name is too long... we need small patch for PortAudio
//	//
//	name.prepend(QString("[%1] ").arg(QString::fromLocal8Bit(dev_api_info->name)));
//
//fini:
//	// shutdown this PA instance
//	pa_err = Pa_TerminateSafe();
//	if (pa_err != paNoError) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] make_sound_user_name(), Pa_TerminateSafe() failed: %d (%s)",
//			pa_err, Pa_GetErrorText(pa_err));
//	}
//
//	return name;
...
// PortAudioWrap commented out by request from CP team
//// filter function used to skip specific devices from list
//static bool is_sound_device_valid(PaDeviceIndex id)
//{
//	const PaDeviceInfo*	dev_info = Pa_GetDeviceInfo(id);
//	if (dev_info == NULL) {
//		WRITE_TRACE(DBG_WARNING,
//			"[HostInfo] Device %d not valid: Pa_GetDeviceInfo() failed",
//			id);
//		return false;
//	}
//
//	const PaHostApiInfo *dev_api_info = Pa_GetHostApiInfo(dev_info->hostApi);
//	if (dev_api_info == NULL) {
//		WRITE_TRACE(DBG_WARNING,
//			"[HostInfo] Device %d not valid: Pa_GetHostApiInfo() failed",
//			id);
//		return false;
//	}
//
//	// check #1, skip "default" ALSA device
//	if (dev_api_info->type == paALSA &&
//			(id == dev_api_info->defaultInputDevice ||
//			id == dev_api_info->defaultOutputDevice)) {
//		WRITE_TRACE(DBG_DEBUG,
//			"[HostInfo] Device %d skipped: ALSA \"default\" device",
//			id);
//		return false;
//	}
//
//	// check #2, skip "default" DirectSound device
//	if (dev_api_info->type == paDirectSound &&
//			(id == dev_api_info->defaultInputDevice ||
//			id == dev_api_info->defaultOutputDevice)) {
//		WRITE_TRACE(DBG_DEBUG,
//			"[HostInfo] Device %d skipped: DirectSound \"default\" device",
//			id);
//		return false;
//	}
//
//	// check #3, skip ALSA plugin devices
//	if (PaAlsa_IsPluginDeviceSafe(id, NULL) == (paNoError + 1)) {
//		WRITE_TRACE(DBG_DEBUG,
//			"[HostInfo] Device %d skipped: ALSA \"plugin\" device",
//			id);
//		return false;
//	}
//
...
// PortAudioWrap commented out by request from CP team
//// filter function used to force "Default" device to be "Null" for some reasons
//static bool is_default_sound_device_valid(PaDeviceIndex id)
//{
//	const char *alsa_plug_type;
//
//	// NOTE: workaround for broken ALSA->PulseAudio here (PWE-4325)
//	if (PaAlsa_IsPluginDeviceSafe(id, &alsa_plug_type) == (paNoError + 1)) {
//		if (strcmp(alsa_plug_type, "pulse") == 0) {
//			WRITE_TRACE(DBG_WARNING,
//				"[HostInfo] Default audio device %d forced to \"Null\", "
//				"ALSA->Pulse workaround",
//				id);
//			return false;
//		}
//	}
...
// PortAudioWrap commented out by request from CP team
//void CDspHostInfo::GetSoundLists()
//{
//	PaDeviceIndex	dev_id;
//	PaDeviceIndex	dev_count;
//	const PaDeviceInfo*	dev_info;
//	QString dev_sys_name;
//	QString dev_user_name;
//
//	PaError pa_err;
//	bool	pa_inited;
//
//	//
//	// Initialize PA here to get always fresh device list
//	//
//	// indirect check for PA already initialized: device list may be obsolete
//	if (Pa_GetDefaultHostApi() != paNotInitialized) {
//		WRITE_TRACE(DBG_WARNING,
//			"[HostInfo] GetSoundLists(): device list may be obsolete");
//	}
//
//	pa_err = Pa_InitializeSafe();
//	if (pa_err != paNoError) {
...
//
//	//
//	// Default playback device, always present in the list
//	//
//	dev_id = get_default_sound_id(false);
//	if (dev_id < 0 && dev_id != paNoDevice) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] get_default_sound_id(\"playback\") failed: %d (%s)",
//			dev_id, Pa_GetErrorText(dev_id));
//		dev_id = paNoDevice;
//	}
//	dev_user_name = make_sound_user_name(paNoDevice, true);
//	dev_sys_name = make_sound_sys_name(dev_id, true);
//	p_HostHwInfo->addSoundOutputDevice(
//			new CHwGenericDevice(PDE_SOUND_DEVICE,
//									dev_user_name,
//									dev_sys_name));
//	WRITE_TRACE(DBG_DEBUG,
//		"[HostInfo] Add sound playback device:\"%s\" (%s)",
//		QSTR2UTF8(dev_user_name),
//		QSTR2UTF8(dev_sys_name));
//
//	//
//	// Default capture device, always present in the list
//	//
//	dev_id = get_default_sound_id(true);
//	if (dev_id < 0 && dev_id != paNoDevice) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] get_default_sound_id(\"capture\") failed: %d (%s)",
//			dev_id, Pa_GetErrorText(dev_id));
//		dev_id = paNoDevice;
//	}
//	dev_user_name = make_sound_user_name(paNoDevice, true);
//	dev_sys_name = make_sound_sys_name(dev_id, true);
//	p_HostHwInfo->addSoundMixerDevice(
//			new CHwGenericDevice(PDE_MIXER_DEVICE,
//									dev_user_name,
//									dev_sys_name));
//	WRITE_TRACE(DBG_DEBUG,
//		"[HostInfo] Add sound capture device:\"%s\" (%s)",
//		QSTR2UTF8(dev_user_name),
//		QSTR2UTF8(dev_sys_name));
//
//	//
//	// get real host devices count (for loop below)
//	//
//	if (pa_inited)
...
//	if (dev_count < 0) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] Pa_GetDeviceCount() failed: %d (%s)",
//			dev_count, Pa_GetErrorText(dev_count));
//
//		dev_count = 0;
//	}
//
//	//
//	// add all host devices to to lists
//	//
//	for (dev_id = 0; dev_id < dev_count; dev_id++) {
//		if (!is_sound_device_valid(dev_id))
//			continue;
//
//		dev_info = Pa_GetDeviceInfo(dev_id);
//
//		dev_user_name = make_sound_user_name(dev_id, false);
//		dev_sys_name = make_sound_sys_name(dev_id, false);
//
...
//
//	//
//	// "Null" devices are always present in lists
//	//
//	dev_user_name = make_sound_user_name(paNoDevice, false);
//	dev_sys_name = make_sound_sys_name(paNoDevice, false);
//	p_HostHwInfo->addSoundOutputDevice(
//			new CHwGenericDevice(PDE_SOUND_DEVICE,
//									dev_user_name,
//									dev_sys_name));
...
//		QSTR2UTF8(dev_sys_name));
//
//	// enable sound if we have at least one real host device
//	if (dev_count > 0)
//		p_HostHwInfo->setSoundDefaultEnabled(1);
//	else
//		p_HostHwInfo->setSoundDefaultEnabled(0);
//
//	//
//	// shutdown PA if needed
//	//
//	if (!pa_inited)
//		return;
//	pa_err = Pa_TerminateSafe();
...
//	if (sleepLocker.isSleeping()) {
//		WRITE_TRACE(DBG_FATAL,
//			"[HostInfo] Unsafe get_default_sound_id(): system is sleeping");
//	}
//
//	// initialize PA here to get fresh device list
//	pa_err = Pa_InitializeSafe();
...
//
//	if (is_input)
//		dev_id = Pa_GetDefaultInputDevice();
//	else
//		dev_id = Pa_GetDefaultOutputDevice();
//
//	// we can force "Default" to be "Null" here...
//	if (!is_default_sound_device_valid(dev_id))
//		dev_id = paNoDevice;
//
//	// shutdown this PA instance
//	pa_err = Pa_TerminateSafe();
...
QString CDspHostInfo::GetDefaultSoundInputSystemName()
{
// PortAudioWrap commented out by request from CP team
//	return make_sound_sys_name(get_default_sound_id(true), true);
	return QString();
}

QString CDspHostInfo::GetDefaultSoundOutputSystemName()
{
// PortAudioWrap commented out by request from CP team
//	return make_sound_sys_name(get_default_sound_id(false), true);
	return QString();
}
