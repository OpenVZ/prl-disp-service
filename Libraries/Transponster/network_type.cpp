...
bool Bridge::load(const QDomElement& src_)
{
	...
	k.push(src_);
	Element<Bridge, Name::Strict<624> > m;
	if (0 > m.consume(k))
		return false;
...
bool Interface::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Interface, Name::Strict<623> > m;
	if (0 > m.consume(k))
		return false;
...
bool Port::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Port, Name::Strict<201> > m;
	if (0 > m.consume(k))
		return false;
...
bool Nat::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Nat, Name::Strict<1174> > m;
	if (0 > m.consume(k))
		return false;
...
bool Forward::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Forward, Name::Strict<1173> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Forward>::parse(Network::Xml::Forward& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1178(m.get<3>().get<0>().getValue());
...
int Traits<Network::Xml::Forward>::generate(const Network::Xml::Forward& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getManaged(), m.get<2>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1178(), m.get<3>().get<0>()))
		return -1;
...
bool Portgroup::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Portgroup, Name::Strict<633> > m;
	if (0 > m.consume(k))
		return false;
...
bool Txt::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Txt, Name::Strict<1186> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Anonymous1206::Anonymous1206(): m_port(), m_priority(), m_weight()
{
}

bool Anonymous1206::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	k.push(src_.firstChildElement());
	return 0 <= Traits<Anonymous1206>::parse(*this, k);
}

bool Anonymous1206::save(QDomElement& dst_) const
{
	return 0 <= Traits<Anonymous1206>::generate(*this, dst_);
}
...
int Traits<Network::Xml::Anonymous1206>::parse(Network::Xml::Anonymous1206& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Network::Xml::Anonymous1206>::generate(const Network::Xml::Anonymous1206& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getDomain(), m.get<0>()))
		return -1;
...
bool Srv::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Srv, Name::Strict<1187> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Srv>::parse(Network::Xml::Srv& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setAnonymous1206(m.get<2>().getValue());
	}
	return output;
}
...
int Traits<Network::Xml::Srv>::generate(const Network::Xml::Srv& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getProtocol(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getAnonymous1206(), m.get<2>()))
		return -1;
...
bool Dns::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dns, Name::Strict<1182> > m;
	if (0 > m.consume(k))
		return false;
...
bool Range::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Range, Name::Strict<1195> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Host1198>::parse(Network::Xml::Host1198& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
	{
		dst_.setChoice1197(m.get<0>().getValue());
		dst_.setName(m.get<1>().getValue());
	}
	return output;
}

int Traits<Network::Xml::Host1198>::generate(const Network::Xml::Host1198& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice1197(), m.get<0>()))
		return -1;
...
bool Bootp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Bootp, Name::Strict<1201> > m;
	if (0 > m.consume(k))
		return false;
...
bool Dhcp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dhcp, Name::Strict<1194> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Ip>::parse(Network::Xml::Ip& dst_, QStack<QDomElement>& stack_)
{
	...
	{
		dst_.setAddress(m.get<0>().getValue());
		dst_.setChoice1192(m.get<1>().getValue());
...
int Traits<Network::Xml::Ip>::generate(const Network::Xml::Ip& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1192(), m.get<1>()))
		return -1;
...
bool Route::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Route, Name::Strict<1175> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Route>::parse(Network::Xml::Route& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1203(m.get<2>().getValue());
...
int Traits<Network::Xml::Route>::generate(const Network::Xml::Route& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1203(), m.get<2>()))
		return -1;
...
bool Network::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Network, Name::Strict<432> > m;
	if (0 > m.consume(k))
		return false;
