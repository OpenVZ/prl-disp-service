...
bool Bridge::load(const QDomElement& src_)
{
	...
	k.push(src_);
	Element<Bridge, Name::Strict<604> > m;
	if (0 > m.consume(k))
		return false;
...
bool Interface::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Interface, Name::Strict<603> > m;
	if (0 > m.consume(k))
		return false;
...
bool Port::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Port, Name::Strict<197> > m;
	if (0 > m.consume(k))
		return false;
...
bool Nat::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Nat, Name::Strict<1123> > m;
	if (0 > m.consume(k))
		return false;
...
bool Forward::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Forward, Name::Strict<1122> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Forward>::parse(Network::Xml::Forward& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1128(m.get<3>().get<0>().getValue());
...
int Traits<Network::Xml::Forward>::generate(const Network::Xml::Forward& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getManaged(), m.get<2>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1128(), m.get<3>().get<0>()))
		return -1;
...
bool Portgroup::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Portgroup, Name::Strict<613> > m;
	if (0 > m.consume(k))
		return false;
...
bool Txt::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Txt, Name::Strict<1136> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Anonymous1157::Anonymous1157(): m_port(), m_priority(), m_weight()
{
}

bool Anonymous1157::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	k.push(src_.firstChildElement());
	return 0 <= Traits<Anonymous1157>::parse(*this, k);
}

bool Anonymous1157::save(QDomElement& dst_) const
{
	return 0 <= Traits<Anonymous1157>::generate(*this, dst_);
}
...
int Traits<Network::Xml::Anonymous1157>::parse(Network::Xml::Anonymous1157& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Network::Xml::Anonymous1157>::generate(const Network::Xml::Anonymous1157& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getDomain(), m.get<0>()))
		return -1;
...
bool Srv::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Srv, Name::Strict<1137> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Srv>::parse(Network::Xml::Srv& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setAnonymous1157(m.get<2>().getValue());
	}
	return output;
}
...
int Traits<Network::Xml::Srv>::generate(const Network::Xml::Srv& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getProtocol(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getAnonymous1157(), m.get<2>()))
		return -1;
...
bool Host::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Host, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Dns::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dns, Name::Strict<1132> > m;
	if (0 > m.consume(k))
		return false;
...
bool Range::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Range, Name::Strict<1146> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Host1149>::parse(Network::Xml::Host1149& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
	{
		dst_.setChoice1148(m.get<0>().getValue());
		dst_.setName(m.get<1>().getValue());
	}
	return output;
}

int Traits<Network::Xml::Host1149>::generate(const Network::Xml::Host1149& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice1148(), m.get<0>()))
		return -1;
...
bool Host1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Host1, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Bootp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Bootp, Name::Strict<1152> > m;
	if (0 > m.consume(k))
		return false;
...
bool Dhcp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dhcp, Name::Strict<1145> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Ip>::parse(Network::Xml::Ip& dst_, QStack<QDomElement>& stack_)
{
	...
	{
		dst_.setAddress(m.get<0>().getValue());
		dst_.setChoice1143(m.get<1>().getValue());
...
int Traits<Network::Xml::Ip>::generate(const Network::Xml::Ip& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1143(), m.get<1>()))
		return -1;
...
bool Route::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Route, Name::Strict<1124> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Route>::parse(Network::Xml::Route& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1154(m.get<2>().getValue());
...
int Traits<Network::Xml::Route>::generate(const Network::Xml::Route& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1154(), m.get<2>()))
		return -1;
...
bool Network::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Network, Name::Strict<412> > m;
	if (0 > m.consume(k))
		return false;
