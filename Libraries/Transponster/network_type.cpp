...
bool Bridge::load(const QDomElement& src_)
{
	...
	k.push(src_);
	Element<Bridge, Name::Strict<634> > m;
	if (0 > m.consume(k))
		return false;
...
bool Interface::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Interface, Name::Strict<635> > m;
	if (0 > m.consume(k))
		return false;
...
bool Address1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Address1, Name::Strict<106> > m;
	if (0 > m.consume(k))
		return false;
...
bool Port::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Port, Name::Strict<207> > m;
	if (0 > m.consume(k))
		return false;
...
bool Nat::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Nat, Name::Strict<1182> > m;
	if (0 > m.consume(k))
		return false;
...
bool Forward::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Forward, Name::Strict<1181> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Forward>::parse(Network::Xml::Forward& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1186(m.get<3>().get<0>().getValue());
...
int Traits<Network::Xml::Forward>::generate(const Network::Xml::Forward& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getManaged(), m.get<2>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1186(), m.get<3>().get<0>()))
		return -1;
...
bool Portgroup::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Portgroup, Name::Strict<632> > m;
	if (0 > m.consume(k))
		return false;
...
bool Txt::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Txt, Name::Strict<1194> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Anonymous1214::Anonymous1214(): m_port(), m_priority(), m_weight()
{
}

bool Anonymous1214::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	k.push(src_.firstChildElement());
	return 0 <= Traits<Anonymous1214>::parse(*this, k);
}

bool Anonymous1214::save(QDomElement& dst_) const
{
	return 0 <= Traits<Anonymous1214>::generate(*this, dst_);
}
...
int Traits<Network::Xml::Anonymous1214>::parse(Network::Xml::Anonymous1214& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Network::Xml::Anonymous1214>::generate(const Network::Xml::Anonymous1214& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getDomain(), m.get<0>()))
		return -1;
...
bool Srv::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Srv, Name::Strict<1195> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Srv>::parse(Network::Xml::Srv& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setAnonymous1214(m.get<2>().getValue());
	}
	return output;
}
...
int Traits<Network::Xml::Srv>::generate(const Network::Xml::Srv& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getProtocol(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getAnonymous1214(), m.get<2>()))
		return -1;
...
bool Dns::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dns, Name::Strict<1190> > m;
	if (0 > m.consume(k))
		return false;
...
bool Range::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Range, Name::Strict<1203> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Host1206>::parse(Network::Xml::Host1206& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
	{
		dst_.setChoice1205(m.get<0>().getValue());
		dst_.setName(m.get<1>().getValue());
	}
	return output;
}

int Traits<Network::Xml::Host1206>::generate(const Network::Xml::Host1206& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice1205(), m.get<0>()))
		return -1;
...
bool Bootp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Bootp, Name::Strict<1209> > m;
	if (0 > m.consume(k))
		return false;
...
bool Dhcp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dhcp, Name::Strict<1202> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Ip>::parse(Network::Xml::Ip& dst_, QStack<QDomElement>& stack_)
{
	...
	{
		dst_.setAddress(m.get<0>().getValue());
		dst_.setChoice1200(m.get<1>().getValue());
...
int Traits<Network::Xml::Ip>::generate(const Network::Xml::Ip& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1200(), m.get<1>()))
		return -1;
...
bool Route::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Route, Name::Strict<1183> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Route>::parse(Network::Xml::Route& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1211(m.get<2>().getValue());
...
int Traits<Network::Xml::Route>::generate(const Network::Xml::Route& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1211(), m.get<2>()))
		return -1;
...
bool Network::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Network, Name::Strict<440> > m;
	if (0 > m.consume(k))
		return false;
