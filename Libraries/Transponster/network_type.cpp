...
bool Bridge::load(const QDomElement& src_)
{
	...
	k.push(src_);
	Element<Bridge, Name::Strict<630> > m;
	if (0 > m.consume(k))
		return false;
...
bool Interface::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Interface, Name::Strict<629> > m;
	if (0 > m.consume(k))
		return false;
...
bool Nat::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Nat, Name::Strict<1178> > m;
	if (0 > m.consume(k))
		return false;
...
bool Forward::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Forward, Name::Strict<1177> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Forward>::parse(Network::Xml::Forward& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1182(m.get<3>().get<0>().getValue());
...
int Traits<Network::Xml::Forward>::generate(const Network::Xml::Forward& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getManaged(), m.get<2>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1182(), m.get<3>().get<0>()))
		return -1;
...
bool Portgroup::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Portgroup, Name::Strict<639> > m;
	if (0 > m.consume(k))
		return false;
...
bool Txt::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Txt, Name::Strict<1190> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Anonymous1210::Anonymous1210(): m_port(), m_priority(), m_weight()
{
}

bool Anonymous1210::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	k.push(src_.firstChildElement());
	return 0 <= Traits<Anonymous1210>::parse(*this, k);
}

bool Anonymous1210::save(QDomElement& dst_) const
{
	return 0 <= Traits<Anonymous1210>::generate(*this, dst_);
}
...
int Traits<Network::Xml::Anonymous1210>::parse(Network::Xml::Anonymous1210& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Network::Xml::Anonymous1210>::generate(const Network::Xml::Anonymous1210& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getDomain(), m.get<0>()))
		return -1;
...
bool Srv::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Srv, Name::Strict<1191> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Srv>::parse(Network::Xml::Srv& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setAnonymous1210(m.get<2>().getValue());
	}
	return output;
}
...
int Traits<Network::Xml::Srv>::generate(const Network::Xml::Srv& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getProtocol(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getAnonymous1210(), m.get<2>()))
		return -1;
...
bool Dns::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dns, Name::Strict<1186> > m;
	if (0 > m.consume(k))
		return false;
...
bool Range::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Range, Name::Strict<1199> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Host1202>::parse(Network::Xml::Host1202& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
	{
		dst_.setChoice1201(m.get<0>().getValue());
		dst_.setName(m.get<1>().getValue());
	}
	return output;
}

int Traits<Network::Xml::Host1202>::generate(const Network::Xml::Host1202& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice1201(), m.get<0>()))
		return -1;
...
bool Bootp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Bootp, Name::Strict<1205> > m;
	if (0 > m.consume(k))
		return false;
...
bool Dhcp::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Dhcp, Name::Strict<1198> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Ip>::parse(Network::Xml::Ip& dst_, QStack<QDomElement>& stack_)
{
	...
	{
		dst_.setAddress(m.get<0>().getValue());
		dst_.setChoice1196(m.get<1>().getValue());
...
int Traits<Network::Xml::Ip>::generate(const Network::Xml::Ip& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1196(), m.get<1>()))
		return -1;
...
bool Route::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Route, Name::Strict<1179> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Network::Xml::Route>::parse(Network::Xml::Route& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1207(m.get<2>().getValue());
...
int Traits<Network::Xml::Route>::generate(const Network::Xml::Route& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1207(), m.get<2>()))
		return -1;
...
bool Network::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Network, Name::Strict<438> > m;
	if (0 > m.consume(k))
		return false;
