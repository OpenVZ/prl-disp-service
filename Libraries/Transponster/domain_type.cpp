...
bool Topology::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Topology, Name::Strict<972> > m;
	if (0 > m.consume(k))
		return false;
...
bool Cell::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Cell, Name::Strict<977> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Cpu947>::parse(Domain::Xml::Cpu947& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Cpu947>::generate(const Domain::Xml::Cpu947& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Feature::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Feature, Name::Strict<966> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Cpu954>::parse(Domain::Xml::Cpu954& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Cpu954>::generate(const Domain::Xml::Cpu954& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getMode(), m.get<0>()))
		return -1;
...
bool Entry::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Entry, Name::Strict<983> > m;
	if (0 > m.consume(k))
		return false;
...
bool Entry1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Entry1, Name::Strict<983> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Clock374>::parse(Domain::Xml::Clock374& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Clock374>::generate(const Domain::Xml::Clock374& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Clock380>::parse(Domain::Xml::Clock380& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Clock380>::generate(const Domain::Xml::Clock380& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getAdjustment(), m.get<1>()))
		return -1;
...
bool Catchup::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Catchup, Name::Strict<409> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Timer389>::parse(Domain::Xml::Timer389& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Timer389>::generate(const Domain::Xml::Timer389& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getName(), m.get<0>()))
		return -1;
...
int Traits<Domain::Xml::Timer396>::parse(Domain::Xml::Timer396& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Timer396>::generate(const Domain::Xml::Timer396& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Timer::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Timer, Name::Strict<382> > m;
	if (0 > m.consume(k))
		return false;
...
bool ScaledInteger::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<ScaledInteger, Name::Strict<319> > m;
	if (0 > m.consume(k))
		return false;
...
bool MaxMemory::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<MaxMemory, Name::Strict<313> > m;
	if (0 > m.consume(k))
		return false;
...
bool Page::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Page, Name::Strict<318> > m;
	if (0 > m.consume(k))
		return false;
...
bool MemoryBacking::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<MemoryBacking, Name::Strict<316> > m;
	if (0 > m.consume(k))
		return false;
...
bool Vcpu::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Vcpu, Name::Strict<323> > m;
	if (0 > m.consume(k))
		return false;
...
bool Device::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Device, Name::Strict<335> > m;
	if (0 > m.consume(k))
		return false;
...
bool Blkiotune::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Blkiotune, Name::Strict<329> > m;
	if (0 > m.consume(k))
		return false;
...
bool Memtune::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Memtune, Name::Strict<330> > m;
	if (0 > m.consume(k))
		return false;
...
bool Vcpupin::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Vcpupin, Name::Strict<353> > m;
	if (0 > m.consume(k))
		return false;
...
bool Iothreadpin::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Iothreadpin, Name::Strict<356> > m;
	if (0 > m.consume(k))
		return false;
...
bool Cputune::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Cputune, Name::Strict<331> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Memory1119>::parse(Domain::Xml::Memory1119& , QStack<QDomElement>& stack_)
{
	marshal_type m;
	return m.consume(stack_);
}

int Traits<Domain::Xml::Memory1119>::generate(const Domain::Xml::Memory1119& , QDomElement& dst_)
{
	marshal_type m;
	return m.produce(dst_);
}
...
bool Memnode::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Memnode, Name::Strict<364> > m;
	if (0 > m.consume(k))
		return false;
...
bool Numatune::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Numatune, Name::Strict<332> > m;
	if (0 > m.consume(k))
		return false;
...
bool Apic::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Apic, Name::Strict<938> > m;
	if (0 > m.consume(k))
		return false;
...
bool Spinlocks::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Spinlocks, Name::Strict<1057> > m;
	if (0 > m.consume(k))
		return false;
...
bool Pvspinlock::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Pvspinlock, Name::Strict<944> > m;
	if (0 > m.consume(k))
		return false;
...
bool Capabilities::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Capabilities, Name::Strict<878> > m;
	if (0 > m.consume(k))
		return false;
...
bool SuspendToMem::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<SuspendToMem, Name::Strict<766> > m;
	if (0 > m.consume(k))
		return false;
...
bool SuspendToDisk::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<SuspendToDisk, Name::Strict<768> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Disk458>::parse(Domain::Xml::Disk458& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Disk458>::generate(const Domain::Xml::Disk458& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Source::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source1, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Host497>::parse(Domain::Xml::Host497& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Host497>::generate(const Domain::Xml::Host497& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Source3::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source3, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source4::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source4, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Driver::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Driver, Name::Strict<525> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
bool Anonymous1120::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	k.push(src_.firstChildElement());
	return 0 <= Traits<Anonymous1120>::parse(*this, k);
}

bool Anonymous1120::save(QDomElement& dst_) const
{
	return 0 <= Traits<Anonymous1120>::generate(*this, dst_);
}
...
int Traits<Domain::Xml::Anonymous1120>::parse(Domain::Xml::Anonymous1120& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Anonymous1120>::generate(const Domain::Xml::Anonymous1120& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Mirror1033>::parse(Domain::Xml::Mirror1033& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
	{
		...
		dst_.setAnonymous1120(m.get<3>().getValue());
	}
	return output;
}

int Traits<Domain::Xml::Mirror1033>::generate(const Domain::Xml::Mirror1033& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getFile(), m.get<0>()))
		return -1;
	...
	if (0 > Details::Marshal::assign(src_.getJob(), m.get<2>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getAnonymous1120(), m.get<3>()))
		return -1;
...
int Traits<Domain::Xml::Mirror1035>::parse(Domain::Xml::Mirror1035& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Mirror1035>::generate(const Domain::Xml::Mirror1035& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Mirror::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Mirror, Name::Strict<1031> > m;
	if (0 > m.consume(k))
		return false;
...
bool Auth::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Auth, Name::Strict<1040> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Variant1048>::parse(Domain::Xml::Variant1048& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Variant1048>::generate(const Domain::Xml::Variant1048& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Variant1052>::parse(Domain::Xml::Variant1052& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Variant1052>::generate(const Domain::Xml::Variant1052& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Iotune::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Iotune, Name::Strict<1046> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Iotune>::parse(Domain::Xml::Iotune& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		dst_.setChoice1049(m.get<0>().getValue());
		dst_.setChoice1053(m.get<1>().getValue());
	}
	return output;
}
...
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice1049(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1053(), m.get<1>()))
		return -1;
...
namespace Xml
{
bool Anonymous1121::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	k.push(src_.firstChildElement());
	return 0 <= Traits<Anonymous1121>::parse(*this, k);
}

bool Anonymous1121::save(QDomElement& dst_) const
{
	return 0 <= Traits<Anonymous1121>::generate(*this, dst_);
}
...
int Traits<Domain::Xml::Anonymous1121>::parse(Domain::Xml::Anonymous1121& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Anonymous1121>::generate(const Domain::Xml::Anonymous1121& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Geometry::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Geometry, Name::Strict<437> > m;
	if (0 > m.consume(k))
		return false;
...
bool Blockio::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Blockio, Name::Strict<522> > m;
	if (0 > m.consume(k))
		return false;
...
bool BackingStore::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<BackingStore, Name::Strict<466> > m;
	if (0 > m.consume(k))
		return false;
...
bool Disk::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Disk, Name::Strict<451> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Variant572>::parse(Domain::Xml::Variant572& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Variant572>::generate(const Domain::Xml::Variant572& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Variant577>::parse(Domain::Xml::Variant577& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Variant577>::generate(const Domain::Xml::Variant577& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getModel(), m.get<0>()))
		return -1;
...
int Traits<Domain::Xml::Variant586>::parse(Domain::Xml::Variant586& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Variant586>::generate(const Domain::Xml::Variant586& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Driver1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Driver1, Name::Strict<525> > m;
	if (0 > m.consume(k))
		return false;
...
bool Controller::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Controller, Name::Strict<551> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Controller>::parse(Domain::Xml::Controller& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice587(m.get<1>().get<2>().getValue());
...
int Traits<Domain::Xml::Controller>::generate(const Domain::Xml::Controller& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getAddress(), m.get<1>().get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice587(), m.get<1>().get<2>()))
		return -1;
...
bool Lease::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Lease, Name::Strict<444> > m;
	if (0 > m.consume(k))
		return false;
...
bool Driver2::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Driver2, Name::Strict<525> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Filesystem593>::parse(Domain::Xml::Filesystem593& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Filesystem593>::generate(const Domain::Xml::Filesystem593& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Filesystem598>::parse(Domain::Xml::Filesystem598& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Filesystem598>::generate(const Domain::Xml::Filesystem598& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getDriver(), m.get<1>()))
		return -1;
...
bool Source5::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source5, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Filesystem603>::parse(Domain::Xml::Filesystem603& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Filesystem603>::generate(const Domain::Xml::Filesystem603& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getDriver(), m.get<1>()))
		return -1;
...
bool Filesystem::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Filesystem, Name::Strict<592> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source6::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source6, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Guest::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Guest, Name::Strict<387> > m;
	if (0 > m.consume(k))
		return false;
...
bool Backend::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Backend, Name::Strict<653> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Driver658>::parse(Domain::Xml::Driver658& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Driver658>::generate(const Domain::Xml::Driver658& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getName(), m.get<0>()))
		return -1;
...
bool Host::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Host, Name::Strict<492> > m;
	if (0 > m.consume(k))
		return false;
...
bool Guest1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Guest1, Name::Strict<387> > m;
	if (0 > m.consume(k))
		return false;
...
bool Driver3::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Driver3, Name::Strict<525> > m;
	if (0 > m.consume(k))
		return false;
...
bool Parameter::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Parameter, Name::Strict<1026> > m;
	if (0 > m.consume(k))
		return false;
...
bool FilterrefNodeAttributes::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<FilterrefNodeAttributes, Name::Strict<666> > m;
	if (0 > m.consume(k))
		return false;
...
bool Tune::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Tune, Name::Strict<668> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Interface619>::parse(Domain::Xml::Interface619& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Interface619>::generate(const Domain::Xml::Interface619& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getSource(), m.get<1>().get<0>()))
		return -1;
...
bool Source7::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source7, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source8::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source8, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source9::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source9, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Interface627>::parse(Domain::Xml::Interface627& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Interface627>::generate(const Domain::Xml::Interface627& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getSource(), m.get<1>().get<0>()))
		return -1;
...
bool Source10::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source10, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Interface633::Interface633(): m_type()
{
}
...
bool Source11::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source11, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Source640>::parse(Domain::Xml::Source640& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Source640>::generate(const Domain::Xml::Source640& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Address644>::parse(Domain::Xml::Address644& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Address644>::generate(const Domain::Xml::Address644& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getBus(), m.get<1>()))
		return -1;
...
bool Source12::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source12, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Interface646>::parse(Domain::Xml::Interface646& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Interface646>::generate(const Domain::Xml::Interface646& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Input::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Input, Name::Strict<859> > m;
	if (0 > m.consume(k))
		return false;
...
bool Sound::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Sound, Name::Strict<825> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source13::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source13, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source14::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source14, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Source887>::parse(Domain::Xml::Source887& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Source887>::generate(const Domain::Xml::Source887& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getProtocol(), m.get<0>()))
		return -1;
...
bool Host1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Host1, Name::Strict<492> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Source888::Source888(): m_protocol()
{
}
...
int Traits<Domain::Xml::Source888>::parse(Domain::Xml::Source888& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Source888>::generate(const Domain::Xml::Source888& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getProtocol(), m.get<0>()))
		return -1;
...
bool Hostdev::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Hostdev, Name::Strict<636> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Hostdev>::parse(Domain::Xml::Hostdev& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		dst_.setChoice869(m.get<0>().getValue());
...
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice869(), m.get<0>()))
		return -1;
...
int Traits<Domain::Xml::Graphics676>::parse(Domain::Xml::Graphics676& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Graphics676>::generate(const Domain::Xml::Graphics676& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Variant685>::parse(Domain::Xml::Variant685& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Variant685>::generate(const Domain::Xml::Variant685& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Listen737>::parse(Domain::Xml::Listen737& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Listen737>::generate(const Domain::Xml::Listen737& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Domain::Xml::Graphics694>::parse(Domain::Xml::Graphics694& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
	{
		dst_.setChoice687(m.get<1>().getValue());
...
int Traits<Domain::Xml::Graphics694>::generate(const Domain::Xml::Graphics694& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice687(), m.get<1>()))
		return -1;
...
bool Channel::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Channel, Name::Strict<704> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Graphics735>::parse(Domain::Xml::Graphics735& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Graphics735>::generate(const Domain::Xml::Graphics735& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getDisplay(), m.get<1>()))
		return -1;
...
bool Acceleration::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Acceleration, Name::Strict<748> > m;
	if (0 > m.consume(k))
		return false;
...
bool Video::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Video, Name::Strict<739> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source15::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source15, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Target2>::parse(Domain::Xml::Target2& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		dst_.setChoice782(m.get<0>().getValue());
		dst_.setPort(m.get<1>().getValue());
	}
	return output;
}
...
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getChoice782(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getPort(), m.get<1>()))
		return -1;
...
bool Qemucdev::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Qemucdev, Name::Strict<433> > m;
	if (0 > m.consume(k))
		return false;
...
bool Channel1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Channel1, Name::Strict<704> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Channel1>::parse(Domain::Xml::Channel1& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice846(m.get<1>().get<1>().getValue());
...
int Traits<Domain::Xml::Channel1>::generate(const Domain::Xml::Channel1& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getQemucdevSrcDef(), m.get<1>().get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice846(), m.get<1>().get<1>()))
		return -1;
...
int Traits<Domain::Xml::Smartcard847>::parse(Domain::Xml::Smartcard847& , QStack<QDomElement>& stack_)
{
	marshal_type m;
	return m.consume(stack_);
}

int Traits<Domain::Xml::Smartcard847>::generate(const Domain::Xml::Smartcard847& , QDomElement& dst_)
{
	marshal_type m;
	return m.produce(dst_);
}
...
int Traits<Domain::Xml::Smartcard851>::parse(Domain::Xml::Smartcard851& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Smartcard851>::generate(const Domain::Xml::Smartcard851& src_, QDomElement& dst_)
{
	marshal_type m;
...
namespace Xml
{
Smartcard852::Smartcard852(): m_type()
{
}
...
int Traits<Domain::Xml::Smartcard852>::parse(Domain::Xml::Smartcard852& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Smartcard852>::generate(const Domain::Xml::Smartcard852& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getType(), m.get<1>()))
		return -1;
...
bool Smartcard::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Smartcard, Name::Strict<710> > m;
	if (0 > m.consume(k))
		return false;
...
bool Redirdev::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Redirdev, Name::Strict<864> > m;
	if (0 > m.consume(k))
		return false;
...
bool Usbdev::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Usbdev, Name::Strict<795> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Backend1021::Backend1021(): m_type()
{
}
...
int Traits<Domain::Xml::Backend1021>::parse(Domain::Xml::Backend1021& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Domain::Xml::Backend1021>::generate(const Domain::Xml::Backend1021& src_, QDomElement& dst_)
{
	marshal_type m;
	if (0 > Details::Marshal::assign(src_.getType(), m.get<1>()))
		return -1;
...
bool Rate::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Rate, Name::Strict<1023> > m;
	if (0 > m.consume(k))
		return false;
...
bool Device1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Device1, Name::Strict<335> > m;
	if (0 > m.consume(k))
		return false;
...
bool Source16::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Source16, Name::Strict<480> > m;
	if (0 > m.consume(k))
		return false;
...
bool Target4::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Target4, Name::Strict<308> > m;
	if (0 > m.consume(k))
		return false;
...
bool Memory2::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Memory2, Name::Strict<311> > m;
	if (0 > m.consume(k))
		return false;
...
bool Watchdog::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Watchdog, Name::Strict<833> > m;
	if (0 > m.consume(k))
		return false;
...
bool Memballoon::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Memballoon, Name::Strict<839> > m;
	if (0 > m.consume(k))
		return false;
...
bool Panic::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Panic, Name::Strict<936> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Domain::Xml::Devices>::parse(Domain::Xml::Devices& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice934List(m.get<1>().getValue());
...
int Traits<Domain::Xml::Devices>::generate(const Domain::Xml::Devices& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getEmulator(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice934List(), m.get<1>()))
		return -1;
...
bool Env::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Env, Name::Strict<1100> > m;
	if (0 > m.consume(k))
		return false;
...
bool Commandline::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Commandline, Name::Scoped<1098, 1101> > m;
	if (0 > m.consume(k))
		return false;
