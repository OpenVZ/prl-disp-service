...
bool Pages::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Pages, Name::Strict<1821> > m;
	if (0 > m.consume(k))
		return false;
...
bool PowerManagement::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<PowerManagement, Name::Strict<1784> > m;
	if (0 > m.consume(k))
		return false;
...
bool MigrationFeatures::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<MigrationFeatures, Name::Strict<1795> > m;
	if (0 > m.consume(k))
		return false;
...
bool ScaledInteger::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<ScaledInteger, Name::Strict<312> > m;
	if (0 > m.consume(k))
		return false;
...
bool Sibling::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Sibling, Name::Strict<1804> > m;
	if (0 > m.consume(k))
		return false;
...
namespace Xml
{
Anonymous1821::Anonymous1821(): m_socketId(), m_coreId()
{
}

bool Anonymous1821::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	k.push(src_.firstChildElement());
	return 0 <= Traits<Anonymous1821>::parse(*this, k);
}

bool Anonymous1821::save(QDomElement& dst_) const
{
	return 0 <= Traits<Anonymous1821>::generate(*this, dst_);
}
...
int Traits<Capability::Xml::Anonymous1821>::parse(Capability::Xml::Anonymous1821& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Capability::Xml::Anonymous1821>::generate(const Capability::Xml::Anonymous1821& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Cpu1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Cpu1, Name::Strict<206> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Capability::Xml::Cpu1>::parse(Capability::Xml::Cpu1& dst_, QStack<QDomElement>& stack_)
{
	...
	{
		dst_.setId(m.get<0>().getValue());
		dst_.setAnonymous1821(m.get<1>().getValue());
	}
	return output;
}
...
int Traits<Capability::Xml::Cpu1>::generate(const Capability::Xml::Cpu1& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getId(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getAnonymous1821(), m.get<1>()))
		return -1;
...
bool Cpus::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Cpus, Name::Strict<990> > m;
	if (0 > m.consume(k))
		return false;
...
bool Cells::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Cells, Name::Strict<1801> > m;
	if (0 > m.consume(k))
		return false;
...
bool Baselabel::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Baselabel, Name::Strict<222> > m;
	if (0 > m.consume(k))
		return false;
...
bool Secmodel::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Secmodel, Name::Strict<1786> > m;
	if (0 > m.consume(k))
		return false;
...
bool Machine::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Machine, Name::Strict<272> > m;
	if (0 > m.consume(k))
		return false;
...
bool Arch::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Arch, Name::Strict<271> > m;
	if (0 > m.consume(k))
		return false;
...
bool Featuretoggle::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Featuretoggle, Name::Strict<1516> > m;
	if (0 > m.consume(k))
		return false;
...
bool Features1::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Features1, Name::Strict<144> > m;
	if (0 > m.consume(k))
		return false;
