...
int Traits<Iface::Xml::Variant1273>::parse(Iface::Xml::Variant1273& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Iface::Xml::Variant1273>::generate(const Iface::Xml::Variant1273& src_, QDomElement& dst_)
{
	marshal_type m;
...
int Traits<Iface::Xml::InterfaceAddressing1268>::parse(Iface::Xml::InterfaceAddressing1268& dst_, QStack<QDomElement>& stack_)
{
	marshal_type m;
	int output = m.consume(stack_);
	if (0 <= output)
...
int Traits<Iface::Xml::InterfaceAddressing1268>::generate(const Iface::Xml::InterfaceAddressing1268& src_, QDomElement& dst_)
{
	marshal_type m;
...
bool Miimon::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Miimon, Name::Strict<1250> > m;
	if (0 > m.consume(k))
		return false;
...
bool Arpmon::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Arpmon, Name::Strict<1258> > m;
	if (0 > m.consume(k))
		return false;
...
bool Bond::load(const QDomElement& src_)
{
	QStack<QDomElement> k;
	k.push(src_);
	Element<Bond, Name::Strict<1241> > m;
	if (0 > m.consume(k))
		return false;
...
int Traits<Iface::Xml::Bond>::parse(Iface::Xml::Bond& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1263(m.get<1>().get<0>().getValue());
...
int Traits<Iface::Xml::Bond>::generate(const Iface::Xml::Bond& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getMode(), m.get<0>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1263(), m.get<1>().get<0>()))
		return -1;
...
int Traits<Iface::Xml::Bridge>::parse(Iface::Xml::Bridge& dst_, QStack<QDomElement>& stack_)
{
	...
	if (0 <= output)
	{
		...
		dst_.setChoice1238List(m.get<2>().getValue());
	}
	return output;
}
...
int Traits<Iface::Xml::Bridge>::generate(const Iface::Xml::Bridge& src_, QDomElement& dst_)
{
	...
	if (0 > Details::Marshal::assign(src_.getDelay(), m.get<1>()))
		return -1;
	if (0 > Details::Marshal::assign(src_.getChoice1238List(), m.get<2>()))
		return -1;
