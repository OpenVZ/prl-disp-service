...
void PrlVmValidateConfigTest::testValidateConfigGeneralParametersOnInvalidRealAndBootCampUndoDisks()
{
...
// Not connected
	...
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice2, PDT_USE_REAL_DEVICE));
////
...
// Not allow real and boot camp disks
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice1, TRUE));
...
// Disable undo disks mode

	CHECK_RET_CODE_EXP(PrlVmCfg_SetUndoDisksMode(m_VmHandle, PUD_DISABLE_UNDO_DISKS));
////
	hJob.reset(PrlVm_ValidateConfig(m_VmHandle, PVC_GENERAL_PARAMETERS));
	CHECK_JOB_RET_CODE(hJob);
}

void PrlVmValidateConfigTest::testValidateConfigGeneralParametersOnInvalidIncompatUndoDisks()
{
	if (TestConfig::isServerModePSBM())
	{
		QSKIP("Undo disks is not available in PSBM !", SkipAll);
...
// Prepare
	...
	QString qsDir1 = qsVmDir + "/hdd_1.hdd";
	QString qsDir2 = qsVmDir + "/hdd_2.hdd";

	CHECK_RET_CODE_EXP(PrlVmCfg_SetOsVersion(m_VmHandle, PVS_GUEST_VER_WIN_2K));

// Undo disks mode

	CHECK_RET_CODE_EXP(PrlVmCfg_SetUndoDisksMode(m_VmHandle, PUD_COMMIT_CHANGES));
	CHECK_RET_CODE_EXP(PrlVmCfg_SetAutoCompressEnabled(m_VmHandle, PRL_FALSE));
...
////
	CHECK_ONE_ERROR_VALIDATION(PVC_GENERAL_PARAMETERS, PRL_ERR_VMCONF_INCOMPAT_HARD_UNDO_DISKS_NOT_ALLOW);
...
void PrlVmValidateConfigTest::testValidateConfigGeneralParametersOnUndoDisksAndNoHardDisksImages()
{
	if (TestConfig::isServerModePSBM())
	{
		QSKIP("Undo disks is not available in PSBM !", SkipAll);
...
// Undo disks mode
	...
	CHECK_RET_CODE_EXP(PrlVmCfg_SetUndoDisksMode(m_VmHandle, PUD_REVERSE_CHANGES));
////
	CHECK_ONE_ERROR_VALIDATION(PVC_GENERAL_PARAMETERS, PRL_ERR_VMCONF_NO_HD_IMAGES_IN_UNDO_DISKS_MODE);

// All hard disks images are not enabled

// 1 Add hard disk
	SdkHandleWrap hDevice1;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_HARD_DISK, hDevice1.GetHandlePtr()));

	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice1, FALSE));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice1, Uuid::createUuid().toString().toUtf8().constData()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetIfaceType(hDevice1, PMS_IDE_DEVICE));
	CHECK_RET_CODE_EXP(PrlVmDev_SetStackIndex(hDevice1, 0));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice1, PDT_USE_IMAGE_FILE));
...
void PrlVmValidateConfigTest::testValidateConfigGeneralParametersOnNoAutoCompressWithUndoDisks()
{
	if (TestConfig::isServerModePSBM())
	{
		QSKIP("Undo disks is not available in PSBM !", SkipAll);
	...
	CHECK_TWO_ERRORS_VALIDATION(PVC_GENERAL_PARAMETERS, PRL_ERR_VMCONF_NO_AUTO_COMPRESS_WITH_UNDO_DISKS,
...
void PrlVmValidateConfigTest::testValidateConfigGeneralParametersOnNoSmartGuardWithUndoDisks()
{
	if (TestConfig::isServerModePSBM())
	{
		QSKIP("Undo disks is not available in PSBM !", SkipAll);
	...
	CHECK_TWO_ERRORS_VALIDATION(PVC_GENERAL_PARAMETERS, PRL_ERR_VMCONF_NO_SMART_GUARD_WITH_UNDO_DISKS,
														PRL_ERR_VMCONF_NO_HD_IMAGES_IN_UNDO_DISKS_MODE);
...
// Remote
	if (TestConfig::isServerMode())//Skip this part of test at non server mode
	{
		CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, TRUE));
		CHECK_RET_CODE_EXP(PrlVmDev_SetConnected(hDevice, TRUE));
...
#define GET_VTD_SUPPORTED_FLAG\
	if ( TestConfig::isServerModePSBM() )\
		QSKIP("Do not examine Vt-d tests", SkipAll);\
	PRL_BOOL bVtdSupported = PRL_FALSE;\
	{\
		SdkHandleWrap hJob(PrlSrv_GetSrvConfig(m_ServerHandle));\
		CHECK_JOB_RET_CODE(hJob);\
		SdkHandleWrap hResult;\
		...
		CHECK_RET_CODE_EXP(PrlResult_GetParam(hResult, hServerConfig.GetHandlePtr()));\
		CHECK_HANDLE_TYPE(hServerConfig, PHT_SERVER_CONFIG);\
		CHECK_RET_CODE_EXP(PrlSrvCfg_IsVtdSupported(hServerConfig, &bVtdSupported));\
	}

void PrlVmValidateConfigTest::testValidateConfigPciGenericDeviceOnCannotAddDevice()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( bVtdSupported )
	{
		QSKIP("Not need Vt-d technology for this test", SkipAll);
		return;
	}

	CHECK_RET_CODE_EXP(PrlSrv_CreateVm(m_ServerHandle, m_VmHandle.GetHandlePtr()));

// 1 Add PCI generic device
	SdkHandleWrap hDevice1;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_PCI_DEVICE, hDevice1.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetStackIndex(hDevice1, 0));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice1, PRL_TRUE));
// 2 Add PCI generic device
	SdkHandleWrap hDevice2;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_PCI_DEVICE, hDevice2.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetStackIndex(hDevice2, 1));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice2, PRL_TRUE));
////
	CHECK_ONE_ERROR_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_CANNOT_BE_ADDED);
////
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice1, PRL_FALSE));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice2, PRL_FALSE));
////
	SdkHandleWrap hJob(PrlVm_ValidateConfig(m_VmHandle, PVC_GENERIC_PCI));
	CHECK_JOB_RET_CODE(hJob);
}

void PrlVmValidateConfigTest::testValidateConfigPciVideoAdapterOnCannotAddDevice()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( bVtdSupported )
	{
...
// 1 Add PCI video adapter
	SdkHandleWrap hDevice1;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice1.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetStackIndex(hDevice1, 0));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice1, PRL_TRUE));
// 2 Add PCI video adapter
	SdkHandleWrap hDevice2;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice2.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetStackIndex(hDevice2, 1));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice2, PRL_TRUE));
////
	CHECK_ONE_ERROR_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_CANNOT_BE_ADDED);
...
void PrlVmValidateConfigTest::testValidateConfigPciNetworkAdapterOnCannotAddDevice()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( bVtdSupported )
	{
...
// 1 Add direct assign network adapter
	SdkHandleWrap hDevice1;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_NETWORK_ADAPTER, hDevice1.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice1, PDT_USE_DIRECT_ASSIGN));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice1, TRUE));
// 2 Add direct assign network adapter
	SdkHandleWrap hDevice2;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_NETWORK_ADAPTER, hDevice2.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice2, PDT_USE_DIRECT_ASSIGN));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice2, TRUE));
////
	CHECK_ONE_ERROR_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_CANNOT_BE_ADDED);
// Not direct assign
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice1, (PRL_VM_DEV_EMULATION_TYPE )PNA_BRIDGED_ETHERNET));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice2, (PRL_VM_DEV_EMULATION_TYPE )COMPATIBLE_ADAPTER_TYPE));
////
	SdkHandleWrap hJob(PrlVm_ValidateConfig(m_VmHandle, PVC_GENERIC_PCI));
	CHECK_JOB_RET_CODE(hJob);
// Direct assign but not enabled
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice1, PDT_USE_DIRECT_ASSIGN));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice2, PDT_USE_DIRECT_ASSIGN));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice1, FALSE));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice2, FALSE));
////
	hJob.reset(PrlVm_ValidateConfig(m_VmHandle, PVC_GENERIC_PCI));
	CHECK_JOB_RET_CODE(hJob);
}

void PrlVmValidateConfigTest::testValidateConfigPciGenericDeviceOnNotExistOrConnect()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
		QSKIP("Need Vt-d technology for this test", SkipAll);
		return;
	}
...
// Hardware info
	...
	PRL_UINT32 nCount = 0;
	CHECK_RET_CODE_EXP(PrlSrvCfg_GetGenericPciDevicesCount(hServerConfig, &nCount));
////
	if ( ! nCount )
	{
		SdkHandleWrap hDevice;
	...
	////
		CHECK_ONE_ERROR_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND);
	}
////
	for(PRL_UINT32 i = 0; i < nCount; i++)
	{
		SdkHandleWrap hHwDevice;
		CHECK_RET_CODE_EXP(PrlSrvCfg_GetGenericPciDevice(hServerConfig, i, hHwDevice.GetHandlePtr()));
		CHECK_HANDLE_TYPE(hHwDevice, PHT_HW_GENERIC_PCI_DEVICE);

		PRL_GENERIC_PCI_DEVICE_CLASS nPciClass = PGD_PCI_OTHER;
		CHECK_RET_CODE_EXP(PrlSrvCfgPci_GetDeviceClass(hHwDevice, &nPciClass));
		if ( nPciClass != PGD_PCI_SOUND && nPciClass != PGD_PCI_OTHER )
			continue;
	...
	////
		...
		QString qsSysName;
		PRL_EXTRACT_STRING_VALUE(qsSysName, hHwDevice, PrlSrvCfgDev_GetId);
		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, QSTR2UTF8(qsSysName)));

		PRL_GENERIC_DEVICE_STATE nDevState;
		CHECK_RET_CODE_EXP(PrlSrvCfgDev_GetDeviceState(hHwDevice, &nDevState));

		if (nDevState == PGS_CONNECTED_TO_VM)
		{
			hJob.reset(PrlVm_ValidateConfig(m_VmHandle, PVC_GENERIC_PCI));
			CHECK_JOB_RET_CODE(hJob);
		}
		...
		{
			CHECK_ONE_ERROR_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_CONNECTED);
		}

		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, QSTR2UTF8(Uuid::createUuid().toString())));
		CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_FALSE));
	////
		hJob.reset(PrlVm_ValidateConfig(m_VmHandle, PVC_GENERIC_PCI));
		CHECK_JOB_RET_CODE(hJob);
	}
...
void PrlVmValidateConfigTest::testValidateConfigPciVideoAdapterOnNotExistOrConnect()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
...
////
	...
	{
		SdkHandleWrap hDevice;
		CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice.GetHandlePtr()));
		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, QSTR2UTF8(Uuid::createUuid().toString())));
		CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));
	////
		CHECK_ONE_ERROR_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND);
...
void PrlVmValidateConfigTest::testValidateConfigPciNetworkAdapterOnNotExistOrConnect()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
...
void PrlVmValidateConfigTest::testValidateConfigPciGenericDevicesOnDuplicateInAnotherVm()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
	...
	{
		INITIALIZE_VM("./TestDspCmdDirValidateVmConfig_vm_config.xml");
		CHECK_RET_CODE_EXP(PrlVmCfg_SetName(m_VmHandle, QSTR2UTF8("machine " + QString::number(i))));
		CHECK_RET_CODE_EXP(PrlVmCfg_SetUuid(m_VmHandle, QSTR2UTF8(Uuid::createUuid().toString())));
		m_JobHandle.reset(PrlVm_Reg(m_VmHandle, "", PRL_TRUE));
		CHECK_JOB_RET_CODE(m_JobHandle);
		...
		SdkHandleWrap hDevice;
		CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_PCI_DEVICE, hDevice.GetHandlePtr()));
		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "1:0:5:any pci device"));
		CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, i < 2 ? PRL_TRUE : PRL_FALSE));
	...
	SdkHandleWrap hDevice;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_PCI_DEVICE, hDevice.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "1:0:6:pci device"));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));
////
	CHECK_TWO_ERRORS_VALIDATION(PVC_GENERIC_PCI,
								PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND,
								PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_DUPLICATE_IN_ANOTHER_VM);
////
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_FALSE));

	m_JobHandle.reset(PrlVm_ValidateConfig(m_VmHandle, PVC_GENERIC_PCI));
	CHECK_JOB_RET_CODE(m_JobHandle);
}

void PrlVmValidateConfigTest::testValidateConfigPciVideoAdaptersOnDuplicateInAnotherVm()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
	...
	for(int i = 0; i < 3; i++)
	{
		...
		SdkHandleWrap hDevice;
		CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice.GetHandlePtr()));
		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "5:2:7:any pci device"));
		CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, i < 2 ? PRL_TRUE : PRL_FALSE));
	...
	SdkHandleWrap hDevice;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "5:2:3:pci device"));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));
////
	CHECK_TWO_ERRORS_VALIDATION(PVC_GENERIC_PCI,
								PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND,
...
void PrlVmValidateConfigTest::testValidateConfigPciNetworkAdaptersOnDuplicateInAnotherVm()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
	...
	for(int i = 0; i < 4; i++)
	{
		...
		SdkHandleWrap hDevice;
		CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_NETWORK_ADAPTER, hDevice.GetHandlePtr()));
		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "3:1:9:any pci device"));
		CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, i != 2 ? PRL_TRUE : PRL_FALSE));
		CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice, i < 3 ? PDT_USE_DIRECT_ASSIGN
																	: PDT_USE_HOST_ONLY_NETWORK));
	...
	SdkHandleWrap hDevice;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_NETWORK_ADAPTER, hDevice.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "3:1:0:pci device"));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDevice, PDT_USE_DIRECT_ASSIGN));
////
	CHECK_TWO_ERRORS_VALIDATION(PVC_GENERIC_PCI,
...
void PrlVmValidateConfigTest::testValidateConfigPciGenericDevicesOnWrongDevice()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
...
// Add as video adapter
		...
		if (nDevState == PGS_CONNECTED_TO_VM)
		{
			CHECK_ONE_ERROR_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_WRONG_DEVICE);
		}
		else
		{
			CHECK_TWO_ERRORS_VALIDATION(PVC_GENERIC_PCI,PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_CONNECTED,
...
void PrlVmValidateConfigTest::testValidateConfigPciVideoAdaptersOnWrongDevice()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
...
void PrlVmValidateConfigTest::testValidateConfigPciNetworkAdaptersOnWrongDevice()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
...
void PrlVmValidateConfigTest::testValidateConfigPciGenericDevicesOnDuplicateSysName()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
	...
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_GENERIC_PCI_DEVICE, hDevice.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "some device"));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));
	...
	CHECK_THREE_ERRORS_VALIDATION(PVC_GENERIC_PCI,
								PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND,
								PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND,
...
void PrlVmValidateConfigTest::testValidateConfigPciVideoAdaptersOnNotSingle()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
	...
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "some device 1"));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));

	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "some device 2"));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));

	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, PDE_PCI_VIDEO_ADAPTER, hDevice.GetHandlePtr()));
	CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDevice, "some device 3"));
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_TRUE));

	CHECK_THREE_ERRORS_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND,
												   PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND,
												   PRL_ERR_VMCONF_GENERIC_PCI_VIDEO_WRONG_COUNT);
////
	CHECK_RET_CODE_EXP(PrlVmDev_SetEnabled(hDevice, PRL_FALSE));

	CHECK_TWO_ERRORS_VALIDATION(PVC_GENERIC_PCI, PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND,
												 PRL_ERR_VMCONF_GENERIC_PCI_DEVICE_NOT_FOUND);
}

void PrlVmValidateConfigTest::testValidateConfigPciNetworkAdaptersOnDuplicateSysName()
{
	GET_VTD_SUPPORTED_FLAG;
	if ( ! bVtdSupported )
	{
...
void PrlVmValidateConfigTest::testValidateConfigWithRemoteDevicesInDesktopAndServerMode()
{
	...
	{
		CHECK_ONE_ERROR_VALIDATION(PVC_ALL, PRL_ERR_VMCONF_DESKTOP_MODE_REMOTE_DEVICES);
	}
}
