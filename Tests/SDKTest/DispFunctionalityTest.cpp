...
namespace {
    ...
    struct EncrInfoStruct
    {
        QWaitCondition m_Condition;
        QMutex m_Mutex;

        PRL_EVENT_TYPE m_expectedEvent;
        EncrInfoStruct( bool bDoEncrypt ): m_expectedEvent( bDoEncrypt
            ? PET_DSP_EVT_VM_ENCRYPT_PROGRESS_CHANGED
            : PET_DSP_EVT_VM_DECRYPT_PROGRESS_CHANGED )
        {}
    };

    void EncryptionCallbackWrap(PRL_RESULT& outRes, PRL_HANDLE _handle, void *pData);
    PRL_RESULT EncryptionCallback(PRL_HANDLE _handle, void *pData)
    {
        PRL_RESULT ret;
        EncryptionCallbackWrap( ret, _handle, pData );
        return ret;
    }
    ...
    void EncryptionCallbackWrap(PRL_RESULT& outRes, PRL_HANDLE _handle, void *pData)
    {
        ...
        EncrInfoStruct *pInfo = static_cast<EncrInfoStruct*>(pData);

        PRL_EVENT_TYPE _event_type;
        CHECK_RET_CODE_EXP(PrlEvent_GetType(hEvent, &_event_type) );
        if(_event_type != pInfo->m_expectedEvent )
        {
            outRes = PRL_ERR_SUCCESS;
            return;
        }
...
void DispFunctionalityTest::testAtomicEdit_EVT_PARAM_VMCFG_TOOLS_NATIVE_LOOK()
{
	// 1. create VM
	// 2. start VM
	// 3. call PrlVm_UpdateToolsSection()
	// 4. get config and check change.
	//////////////////////////////////////////////////////////////////////////
	...
	// 3. Update field through PrlVm_UpdateToolsSection()
	CVmConfiguration vmConfig;
	EXTRACT_CONFIG_AS_XML_MODEL_OBJECT( m_VmHandle, (&vmConfig) );

	SmartPtr<CVmNativeLook>
		pNewNativeLook( new CVmNativeLook( vmConfig.getVmSettings()->getVmTools()->getNativeLook()) );
	pNewNativeLook->setEnabled( !pNewNativeLook->isEnabled() );

	// 3.2 call PrlVm_UpdateToolsSection()
	hJob.reset(PrlVm_UpdateToolsSection( m_VmHandle, QSTR2UTF8( pNewNativeLook->toString() ) ));
	CHECK_JOB_RET_CODE(hJob);
	...
	EXTRACT_CONFIG_AS_XML_MODEL_OBJECT( m_VmHandle, (&vmConfig) );
	QCOMPARE( vmConfig.getVmSettings()->getVmTools()->getNativeLook()->toString(), pNewNativeLook->toString() );
}
...
void DispFunctionalityTest::testVmEncryption_RegisterEncryptedVm()
{
	QSKIP( "Not implemented yet", SkipAll );
}
...
QString DispFunctionalityTest::getPluginUuidByType( EncryptionPluginType pluginType )
{
	switch( pluginType )
	{
	case eptDefault:
		return "";
	case eptTestPlugin1:
		return DefaultParams::getTestPluginUuid();
	case eptTestPlugin2:
		return DefaultParams::getTestPlugin2Uuid();
	default:
		PRL_ASSERT(0);
		return "";
	}
...
void DispFunctionalityTest::createEncryptedVm(
	bool& bRes /*out*/,  SdkHandleWrap& hVm/*out*/
	, const SdkHandleWrap& hServerHandle, const QString& sVmName
	, EncryptionPluginType pluginType, quint32 nFlags )
{
	QString sVmUuid;
	SdkHandleWrap hJob;

	QString sPassword = DefaultParams::getVmPasswordStr();
	QString sPluginId = getPluginUuidByType(pluginType);

	bRes = false;
	createEncryptedVm( bRes, hServerHandle, sVmName
		, sPassword, sPluginId, nFlags, hVm, sVmUuid );
	QVERIFY(bRes);
	bRes = true;
}


void DispFunctionalityTest::createEncryptedVm( const QString& sVmName
											  ...
											  , QString& sVmUuid /*out*/ )
{
	return createEncryptedVm( bRes, m_ServerHandle
		, sVmName, sPassword, "", createEncriptedFlags, hVmHandle, sVmUuid );
}

void DispFunctionalityTest::createVmAndPrepareForEncrypt( bool& bRes /*out*/
														 ...
														 , QString& sVmUuid /*out*/ )
{
	...
	SdkHandleWrap hJob;
	createVm( hServerHandle, sVmName, bRes, hVmHandle, sVmUuid );
	QVERIFY( bRes );
	bRes = false;

	if( createEncriptedFlags )
	{
		if( createEncriptedFlags & cemWithPlainHdd )
		{
			SdkHandleWrap hSrvConfig;
			GET_SRV_CONFIG( hServerHandle, hSrvConfig );

			bRes = false;
			SdkHandleWrap hHddHandle;
			AddHdd( bRes, hVmHandle, 200, PHD_PLAIN_HARD_DISK, hSrvConfig, hHddHandle);
			QVERIFY(bRes);
		}


		// create mem files.
		hJob.reset(PrlVm_Start(hVmHandle));
		CHECK_JOB_RET_CODE(hJob);

		AutoStopVm autoStopper(hVmHandle);

		if( createEncriptedFlags & cemMemFromSnapshots )
		{
			// workaround for #PSBM-22837
			HostUtils::Sleep(500);

			hJob.reset(PrlVm_CreateSnapshot( hVmHandle, "AAAA", "AAA" ));
			CHECK_JOB_RET_CODE(hJob);

			// we have race
			HostUtils::Sleep(1500);
		}

		PRL_RESULT nRetCode;
		quint32 nMaxIterations = 60;
		do
		{
			if( createEncriptedFlags & cemMemFromSuspend )
				hJob.reset(PrlVm_Suspend(hVmHandle));
			else
				hJob.reset(PrlVm_Stop(hVmHandle, PRL_FALSE));
			GET_JOB_RET_CODE_EX(hJob, nRetCode);
			if ( PRL_FAILED(nRetCode) )
			{
				if ( PRL_ERR_DISP_VM_IS_NOT_STARTED == nRetCode )
					QFAIL("Unexpected return code");
				else
					QTest::qSleep(1000);
			}
		} while( PRL_FAILED(nRetCode) && 0 != --nMaxIterations );

		if ( PRL_FAILED(nRetCode) )
			QFAIL("Stop/suspend VM procedure wasn't successful");
	}
	bRes = true;
}

void DispFunctionalityTest::createEncryptedVm( bool& bRes /*out*/
											  , const SdkHandleWrap& hServerHandle
											  , const QString& sVmName
											  , const QString& sPassword
											  ...
											  , QString& sVmUuid /*out*/ )
{
	...
	bRes = false;
	createVmAndPrepareForEncrypt( bRes, hServerHandle, sVmName
		, createEncriptedFlags, hVmHandle, sVmUuid );
	QVERIFY(bRes);

	hJob.reset(PrlVm_Encrypt(hVmHandle
		, QSTR2UTF8(sPassword)
		, sPluginId.isEmpty()
		? NULL
		:QSTR2UTF8(sPluginId)
	...
	// check that new password was hashed
	hJob.reset(PrlVm_GetStatistics(hVmHandle));
	CHECK_JOB_RET_CODE(hJob);

	bRes = true;
}

void DispFunctionalityTest::testVmEncryption_testVmEncrypt()
{
	...
	quint32 uiFlags = cemMemFromSnapshots |  cemMemFromSuspend;
	createEncryptedVm( GEN_VM_NAME_BY_TEST_FUNCTION(), sPassword, uiFlags, bRes, m_VmHandle, sVmUuid );
...
void DispFunctionalityTest::testVmEncryption_testVmDecrypt()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	QString sPasswordWrong = sPassword + "xxx";
	hJob.reset(PrlVm_Decrypt(m_VmHandle, QSTR2UTF8(sPasswordWrong), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_WRONG_PASSWORD_TO_ENCRYPTED_VM);

	hJob.reset(PrlVm_Decrypt(m_VmHandle, QSTR2UTF8(sPassword), 0 ));
	CHECK_JOB_RET_CODE(hJob);
}

void DispFunctionalityTest::testVmEncryption_testVmDecryptFromAnotherSession()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	SdkHandleWrap hVmHandle2 = session2.GetVmByUuid( sVmUuid );
	QVERIFY( hVmHandle2.valid() );

	hJob.reset(PrlVm_Decrypt(hVmHandle2, QSTR2UTF8(sPassword), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM );

	hJob.reset(PrlVm_Authorise(hVmHandle2, QSTR2UTF8(sPassword), 0 ));
	CHECK_JOB_RET_CODE(hJob);
...
void DispFunctionalityTest::testVmEncryption_testAlreadyEncryptVm()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	bool bRes;
	QString sPassword = "1q2w3e";
	quint32 uiFlags = cemWithoutMemFiles;
	createEncryptedVm( GEN_VM_NAME_BY_TEST_FUNCTION(), sPassword, uiFlags, bRes, m_VmHandle, sVmUuid );
	AutoDeleteVm autoDeleter(m_VmHandle);
	QVERIFY( bRes );

	hJob.reset(PrlVm_Encrypt(m_VmHandle, QSTR2UTF8(sPassword), NULL, 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_VM_ALREADY_ENCRYPTED);
}

void DispFunctionalityTest::testVmEncryption_testAlreadyDecryptVm()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	hJob.reset(PrlVm_Decrypt(m_VmHandle, QSTR2UTF8(sPassword), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_UNABLE_TO_DECRYPT_UNENCRYPTED_VM );
}

void DispFunctionalityTest::testVmEncryption_testChangeVmPassword()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	QString sPasswordNew = "1q2w3e4r";
	hJob.reset(PrlVm_ChangePassword(m_VmHandle, QSTR2UTF8(sPassword), QSTR2UTF8(sPasswordNew), 0 ));
	CHECK_JOB_RET_CODE(hJob);
	...
	hJob.reset(PrlVm_GetStatistics( hVmHandle2 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM );
	...
	hJob.reset(PrlVm_Authorise(hVmHandle2, QSTR2UTF8(sPasswordNew), 0 ));
	CHECK_JOB_RET_CODE(hJob);
...
void DispFunctionalityTest::testVmEncryption_testStartVm()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED

		// 0. create encrypted vm
		// 1. test start from the same session
		// 2. start from another session without auth
		// 3. start from another session with auth
	...
	// 1. test start from the same session
	{
		...
		AutoStopVm autoStopper(m_VmHandle);
	}
	...
	// 2. start from another session without auth
	{
		...
		hJob.reset(PrlVm_Start(hVmHandle2));
		CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM);
	}
	...
	// 3. start from another session with auth
	{
		...
		AutoStopVm autoStopper(hVmHandle2);
...
void DispFunctionalityTest::testVmEncryption_testStartVm_WithMemFiles()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
...
void DispFunctionalityTest::testVmEncryption_testCancelOnDisconnect()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
		...
		// disconnect session.
		// wait
		// check advanced status && isEncrypted
	...
	//////////////////////////////////////////////////////////////////////////
	// add plain hdd
	AddPlainHdd( hVmHandle, 200, bRes );
	QVERIFY( bRes );
	...
	// disconnect session.
	{
        ...
        SmartPtr<EncrInfoStruct> pEncrInfo( new EncrInfoStruct(true) );
        QMutexLocker _lock(&pEncrInfo->m_Mutex);
        CHECK_RET_CODE_EXP(PrlVm_RegEventHandler(hVmHandle2, EncryptionCallback, pEncrInfo.getImpl()));

        hJob.reset(PrlVm_Encrypt(hVmHandle2, "1q2w3e", NULL, PACF_CANCEL_TASK_ON_END_SESSION ));

        // wait for first progress
        bRes = pEncrInfo->m_Condition.wait(&pEncrInfo->m_Mutex, PRL_JOB_WAIT_TIMEOUT);
        CHECK_RET_CODE_EXP(PrlVm_UnregEventHandler(hVmHandle2, EncryptionCallback, pEncrInfo.getImpl())) ;
        QVERIFY(bRes);
    }

	PRL_UINT64 nTimeoutTimeStamp = PrlGetTickCount64() + 60*PrlGetTicksPerSecond();
	PRL_BOOL bIsEncrypted = false;
	VIRTUAL_MACHINE_ADDITION_STATE nAdvState;
	do
	{
		HostUtils::Sleep( 500 );
		...
		SdkHandleWrap hVmInfo;
		CHECK_RET_CODE_EXP( PrlVmCfg_GetVmInfo( hVmHandle, hVmInfo.GetHandlePtr() ) );
		CHECK_RET_CODE_EXP( PrlVmInfo_GetAdditionState( hVmInfo, &nAdvState ) );
		CHECK_RET_CODE_EXP( PrlVmCfg_IsEncrypted(hVmHandle, &bIsEncrypted ) );

		QVERIFY( PrlGetTickCount64() < nTimeoutTimeStamp );

	}while( nAdvState != VMAS_NOSTATE );

	QCOMPARE( bIsEncrypted, (PRL_BOOL)PRL_FALSE );
}

void DispFunctionalityTest::testVmEncryption_testNoCancelOnDisconnect_WithoutFlag()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// disconnect session.
	{
		...
		hJob.reset(PrlVm_Encrypt(hVmHandle2, "1q2w3e", NULL, 0 ));
		// wait for start task
	...
	QCOMPARE( bIsEncrypted, (PRL_BOOL)PRL_TRUE );
}

void DispFunctionalityTest::testVmEncryption_testFlag_CheckFreesizeOnly()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED

		// create vm
		// add plain hdd
		// start vm encrypt with flag
		// check isEncrypted state.
	...
	hJob.reset(PrlVm_Encrypt(hVmHandle, "1q2w3e", NULL, PEF_CHECK_PRECONDITIONS_ONLY ));
	CHECK_JOB_RET_CODE(hJob);

	hJob.reset(PrlVm_RefreshConfig(hVmHandle));
	CHECK_JOB_RET_CODE(hJob);

	PRL_BOOL bIsEncrypted;
	CHECK_RET_CODE_EXP( PrlVmCfg_IsEncrypted( hVmHandle, &bIsEncrypted ) );
	QCOMPARE( bIsEncrypted, PRL_BOOL(false) );
}
...
void DispFunctionalityTest::testAtomicEdit_EVT_PARAM_VMCFG_TOOLS_COHERENCE()
{
	// 1. create VM
	// 2. start VM
	// 3. call PrlVm_UpdateToolsSection()
	...
	SmartPtr<CVmCoherence> pNewVmCoherence(
		new CVmCoherence( vmConfig.getVmSettings()->getVmTools()->getVmCoherence() ) );
	pNewVmCoherence->setShowTaskBar( !pNewVmCoherence->isShowTaskBar() );

	// 3.2 call PrlVm_UpdateToolsSection()
	hJob.reset(PrlVm_UpdateToolsSection( m_VmHandle, QSTR2UTF8( pNewVmCoherence->toString() ) ));
	CHECK_JOB_RET_CODE(hJob);
	...
	EXTRACT_CONFIG_AS_XML_MODEL_OBJECT( m_VmHandle, (&vmConfig) );
	QCOMPARE( vmConfig.getVmSettings()->getVmTools()->getVmCoherence()->toString(), pNewVmCoherence->toString() );
}


void DispFunctionalityTest::testAtomicEdit_EVT_PARAM_VMCFG_TOOLS_SHARED_FOLDERS()
{
	// 1. create VM
	// 2. start VM
	// 3. call PrlVm_UpdateToolsSection()
	...
	SmartPtr<CVmSharing> pNewVmSharing( new CVmSharing( vmConfig.getVmSettings()->getVmTools()->getVmSharing() ) );
	pNewVmSharing->getHostSharing()->setEnabled( !pNewVmSharing->getHostSharing()->isEnabled() );

	// 3.2 call PrlVm_UpdateToolsSection()
	hJob.reset(PrlVm_UpdateToolsSection( m_VmHandle, QSTR2UTF8( pNewVmSharing->toString() ) ));
	CHECK_JOB_RET_CODE(hJob);
	...
	EXTRACT_CONFIG_AS_XML_MODEL_OBJECT( m_VmHandle, (&vmConfig) );
	QCOMPARE( vmConfig.getVmSettings()->getVmTools()->getVmSharing()->toString(), pNewVmSharing->toString() );
}


void DispFunctionalityTest::testAtomicEdit_EVT_PARAM_VMCFG_TOOLS_SHARED_PROFILES()
{
	// 1. create VM
	// 2. start VM
	// 3. call PrlVm_UpdateToolsSection()
	...
	SmartPtr<CVmSharedProfile>
		pNewVmSharedProfile(new CVmSharedProfile( vmConfig.getVmSettings()->getVmTools()->getVmSharedProfile() ) );
	pNewVmSharedProfile->setUseDocuments( !pNewVmSharedProfile->isUseDocuments() );

	// 3.2 call PrlVm_UpdateToolsSection()
	hJob.reset(PrlVm_UpdateToolsSection( m_VmHandle, QSTR2UTF8( pNewVmSharedProfile->toString() ) ));
	CHECK_JOB_RET_CODE(hJob);
	...
	EXTRACT_CONFIG_AS_XML_MODEL_OBJECT( m_VmHandle, (&vmConfig) );
	QCOMPARE( vmConfig.getVmSettings()->getVmTools()->getVmSharedProfile()->toString()
		, pNewVmSharedProfile->toString() );
}


void DispFunctionalityTest::testAtomicEdit_EVT_PARAM_VMCFG_TOOLS_SHARED_APPLICATIONS()
{
	// 1. create VM
	// 2. start VM
	// 3. call PrlVm_UpdateToolsSection()
	...
	SmartPtr<CVmSharedApplications>
		pNewVmSharedApplications( new CVmSharedApplications(
		vmConfig.getVmSettings()->getVmTools()->getVmSharedApplications() ) );
	pNewVmSharedApplications->setSmartSelect( !pNewVmSharedApplications->isSmartSelect()  );

	// 3.2 call PrlVm_UpdateToolsSection()
	hJob.reset(PrlVm_UpdateToolsSection( m_VmHandle, QSTR2UTF8( pNewVmSharedApplications->toString() ) ));
	CHECK_JOB_RET_CODE(hJob);
	...
	EXTRACT_CONFIG_AS_XML_MODEL_OBJECT( m_VmHandle, (&vmConfig) );
	QCOMPARE( vmConfig.getVmSettings()->getVmTools()->getVmSharedApplications()->toString()
		, pNewVmSharedApplications->toString() );
}
...
void DispFunctionalityTest::switchPluginsOnOrOff( bool& bRes
	 , const SdkHandleWrap& hServer, PluginsSwitchAction action
	 , bool bRevertAlreadyState, PRL_RESULT nExpectedResult, bool bSendTestFlags )
{
	bRes = false;

	SdkHandleWrap hDispConfig;

	PRL_BOOL bDoEnable = (psaEnable==action)?PRL_TRUE:PRL_FALSE;

	GET_RESULT_AFTER_ASYNC_CALL( (PrlSrv_GetCommonPrefs(hServer)), hDispConfig );

	PRL_BOOL bEnabled;
	CHECK_RET_CODE_EXP(PrlDispCfg_ArePluginsEnabled(hDispConfig, &bEnabled ));
	PRL_BOOL bEnabledBeforeChanges = bEnabled;

	if( bEnabled == bDoEnable && bRevertAlreadyState)
	{
		WRITE_TRACE( DBG_DEBUG, "Enabled is already set( to %d). We revert it before continue", bEnabled );

		CHECK_JOB_RET_CODE( PrlSrv_CommonPrefsBeginEdit(hServer) );
		CHECK_RET_CODE_EXP(PrlDispCfg_EnablePlugins(hDispConfig, !bEnabled));
		CHECK_JOB_RET_CODE( PrlSrv_CommonPrefsCommitEx(hServer, hDispConfig
			, IFV_TEST_COMMIT_COMMON_PREFS_TO_ENABLE_PLUGINS) );


		GET_RESULT_AFTER_ASYNC_CALL( (PrlSrv_GetCommonPrefs(hServer)), hDispConfig );
		CHECK_RET_CODE_EXP(PrlDispCfg_ArePluginsEnabled(hDispConfig, &bEnabled ));
		QCOMPARE( (bool)bEnabled, !(bool)bDoEnable );
	}

	PRL_RESULT nBeginEditRetCode =
		(nExpectedResult==PRL_ERR_USER_NO_AUTH_TO_EDIT_SERVER_SETTINGS)
		? PRL_ERR_USER_NO_AUTH_TO_EDIT_SERVER_SETTINGS
		: PRL_ERR_SUCCESS;
	CHECK_ASYNC_OP_FAILED( PrlSrv_CommonPrefsBeginEdit(hServer), nBeginEditRetCode );

	CHECK_RET_CODE_EXP(PrlDispCfg_EnablePlugins(hDispConfig, bDoEnable));
	PRL_UINT32 nFlags = bSendTestFlags
		?(PRL_UINT32)IFV_TEST_COMMIT_COMMON_PREFS_TO_ENABLE_PLUGINS
		:0;
	CHECK_ASYNC_OP_FAILED( PrlSrv_CommonPrefsCommitEx(hServer, hDispConfig, nFlags)\
		,nExpectedResult );

	// check it
	GET_RESULT_AFTER_ASYNC_CALL( (PrlSrv_GetCommonPrefs(hServer)), hDispConfig );
	CHECK_RET_CODE_EXP(PrlDispCfg_ArePluginsEnabled(hDispConfig, &bEnabled ));
	if( PRL_FAILED(nExpectedResult) )
		QCOMPARE( (bool)bEnabled, (bool)bEnabledBeforeChanges );
	else
		QCOMPARE( (bool)bEnabled, (bool)bDoEnable );
...
static QStringList getTestPluginPaths( quint32 pluginsMask )
{
	QStringList lstFiles;
	QString sPathToPluginDll  = QCoreApplication::applicationDirPath();
#ifdef _MAC_
	sPathToPluginDll += "/../../../"; // xxx.app/Contents/MacOS
#endif

	QDir	cDir(sPathToPluginDll);
	QFileInfoList cFileList;
	cDir.setFilter(QDir::AllEntries | QDir::Hidden | QDir::System /*| QDir::SymLinks*/ );
	QStringList lstLibNames;

	QStringList lstNames;
	if( pluginsMask & DispFunctionalityTest::eptTestPlugin1 )
		lstNames << "test_plugin_example";
	if( pluginsMask & DispFunctionalityTest::eptTestPlugin2 )
		lstNames << "test_plugin_example2";

	PRL_ASSERT(lstNames.size());

	foreach( QString sName, lstNames )
	{
		lstLibNames <<
#		ifndef _WIN_
		QString( "lib" ) +
#		endif
	...
	cDir.setNameFilters( lstLibNames );
	...
	for(int i = 0; i < cFileList.size();i++ )
	{
		if( cFileList.at(i).isFile() )
			lstFiles.append( cFileList.at(i).absoluteFilePath() );
	}
...
namespace{
	QString g_sLastPathToPluginDir;

	QString getCurrentPluginsDir() { return g_sLastPathToPluginDir; }

	// should be static because on windows DLL is busy to delete till unload
	//		(  till test finished )
	AutoDirRemover g_AutoRemoverTestPluginsDir;
};

static void removePluginFromPluginDir( bool& bRes, quint32 pluginsMask, const QString& sPathToPluginDir )
{
	bRes = false;
	QStringList lstPlugins = getTestPluginPaths( pluginsMask );
	QVERIFY( !lstPlugins.isEmpty() );

	foreach( QString sPath, lstPlugins )
	{
		QString sNewPath = QString("%1/%2")
			.arg(sPathToPluginDir)
			.arg( QFileInfo(sPath).fileName() );

		if( QFile::remove(sNewPath) )
			continue;
		WRITE_TRACE( DBG_FATAL, "Unable to remove file %s ", QSTR2UTF8(sNewPath) );
		bRes = false;
		return;
	}
	bRes = true;
...
static void copyPluginToPluginDir( bool& bRes, quint32 pluginsMask, const QString& sPathToPluginDir
	, bool bMakeSecondCopy = false // use in tests which load two plugins with same ID
	)
{
	bRes = false;
	QStringList lstPlugins = getTestPluginPaths( pluginsMask );
	...
	if( bMakeSecondCopy )
	{
		QString sTmpDir = GENERATE_PATH_TO_TEMP_DIR();
		QVERIFY( QDir().mkdir( sTmpDir ) );
		QVERIFY( g_AutoRemoverTestPluginsDir.add( sTmpDir ) );

		QString sDupName = QFileInfo( lstPlugins[0] ).fileName();
		int nDotPos = sDupName.lastIndexOf( "." );
		QVERIFY( nDotPos!=-1 );
		sDupName.insert( nDotPos, ".dup");

		QString sNewPath = sTmpDir + "/" + sDupName;
		QVERIFY( QFile::copy( lstPlugins[0], sNewPath ) );
	...
	foreach( QString sPath, lstPlugins )
	{
		...
		bool bOk = QFile::copy( sPath, sNewPath );
		if( !bOk )
		{
			WRITE_TRACE( DBG_FATAL, "Unable copy file '%s' to '%s'"
				, QSTR2UTF8(sPath), QSTR2UTF8(sNewPath) );
			QVERIFY( bOk );
		}
#ifndef _WIN_
		if (::chown( QSTR2UTF8(sNewPath), 0, 0) == -1)
		{
			WRITE_TRACE(DBG_FATAL, "chown call was failed for file '%s' with error code %d"
				, QSTR2UTF8(sNewPath), errno);
			QVERIFY(0);
		}
		if (::chmod( QSTR2UTF8(sNewPath), 0555/*-r-xr-xr-x*/ ) == -1)
		{
			WRITE_TRACE(DBG_FATAL, "chmod call was failed for file '%s' with error code %d"
				, QSTR2UTF8(sNewPath), errno);
			QVERIFY(0);
		}
#endif
		WRITE_TRACE( DBG_DEBUG, "plugin: '%s'", QSTR2UTF8(sNewPath) );

	}
	bRes = true;
}

void DispFunctionalityTest::loadTestPlugin( bool& bRes
	, const SdkHandleWrap& hServer, quint32 pluginsMask, bool bLoadCopyWithSameId )
{
	//////////////////////////////////////////////////////////////////////////
	// create tmp dir. copy plugin to it, load plugin; delete dir;

	bRes = false;

	QString sPathToPluginDir = GENERATE_PATH_TO_TEMP_DIR();
	QVERIFY( QDir().mkdir( sPathToPluginDir ) );
	QVERIFY( g_AutoRemoverTestPluginsDir.add( sPathToPluginDir ) );

	copyPluginToPluginDir( bRes, pluginsMask, sPathToPluginDir, bLoadCopyWithSameId );
	QVERIFY( bRes );

	SdkHandleWrap hJob(PrlSrv_StoreValueByKey(
		hServer, PRL_KEY_TO_LOAD_PLUGIN_FROM_DIR, QSTR2UTF8(sPathToPluginDir), 0 ));

	PRL_RESULT nExpectedError = PRL_ERR_SUCCESS;

#ifndef EXTERNALLY_AVAILABLE_BUILD

	if( bLoadCopyWithSameId )
	{
		// NOTE: dispatcher skip wrongs plugins, to continue load another plugins.
		//	So it returns PRL_ERR_SUCCESS instead PRL_ERR_OBJECT_DUPLICATE_UID.
		nExpectedError = PRL_ERR_SUCCESS; // PRL_ERR_OBJECT_DUPLICATE_UID;
	}

#else
	nExpectedError = PRL_ERR_UNIMPLEMENTED;
#endif

	CHECK_ASYNC_OP_FAILED(hJob, nExpectedError );

	g_sLastPathToPluginDir = sPathToPluginDir;

	bRes = true;
}

void DispFunctionalityTest::checkEncryptFailure( bool& bRes
	...
	, PRL_RESULT nExpectedError )
{
	...
	createVm( hServer, sVmName, bRes, hVmHandle, sVmUuid );
	AutoDeleteVm autoDeleter(hVmHandle);
	QVERIFY( bRes );

	CHECK_ASYNC_OP_FAILED( PrlVm_Encrypt(hVmHandle, "123", QSTR2UTF8( getPluginUuidByType(pluginType) ), 0 )
		, nExpectedError );
...
void DispFunctionalityTest::doEncryptDecryptPair( bool& bRes
	 , const SdkHandleWrap& hServer, const QString& sVmName, EncryptionPluginType pluginType )
{
	SdkHandleWrap hJob;
	SdkHandleWrap hVmHandle;
	...
	// encrypt
	bRes = false;
	createEncryptedVm( bRes, hVmHandle, hServer, sVmName, pluginType );
	AutoDeleteVm autoDeleter(hVmHandle);
	QVERIFY( bRes );
	...
	QString sPassword = DefaultParams::getVmPasswordStr();
	QString sPasswordWrong = sPassword + "xxx";
	hJob.reset(PrlVm_Decrypt(hVmHandle, QSTR2UTF8(sPasswordWrong), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_WRONG_PASSWORD_TO_ENCRYPTED_VM);

	hJob.reset(PrlVm_Decrypt(hVmHandle, QSTR2UTF8(sPassword), 0 ));
	CHECK_JOB_RET_CODE(hJob);
}
...
namespace{
	bool isExternallyAvailableBuild()
	{
...
#define SKIP_FOR_EXTERNAL_BUILD() \
{ \
	if( isExternallyAvailableBuild() ) { \
	QSKIP( "Logic to load test plugin is disabled for external builds", SkipAll ); \
	return; \
	} \
}


//  load test plugin from security hole
#define LOAD_TEST_PLUGIN( hServer ) \
{ \
	bool bRes = false; \
	loadTestPlugin(bRes, hServer, eptTestPlugin1); \
	QVERIFY(bRes); \
	\
	SKIP_FOR_EXTERNAL_BUILD(); \
}

#define LOAD_ALL_TEST_PLUGINS( hServer ) \
{ \
	bool bRes = false; \
	loadTestPlugin(bRes, hServer, eptTestPlugin1 | eptTestPlugin2 ); \
	QVERIFY(bRes); \
	\
	SKIP_FOR_EXTERNAL_BUILD(); \
}

#define LOAD_TEST_PLUGINS_WITH_DUP_ID( hServer, pluginType ) \
{ \
	bool bRes = false; \
	loadTestPlugin(bRes, hServer, pluginType, true ); \
	QVERIFY(bRes); \
	\
	SKIP_FOR_EXTERNAL_BUILD(); \
}

void DispFunctionalityTest::testEncryptionPluginsEnable()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	SimpleServerWrapper session(0);
	AutoRestoreCommonPrefs autoCommonPresRestorer(session);

	//////////////////////////////////////////////////////////////////////////
	// enable plugins
	bRes = false;
	switchPluginsOnOrOff(bRes, session, psaEnable, true);
	QVERIFY(bRes);

	// load test plugin from security hole
	LOAD_TEST_PLUGIN( session );

	//////////////////////////////////////////////////////////////////////////
	// do test operations
	bRes = false;
	doEncryptDecryptPair( bRes, session, GEN_VM_NAME_BY_TEST_FUNCTION(), eptTestPlugin1 );
	QVERIFY(bRes);
}
...
void DispFunctionalityTest::testEncryptionPluginsEnable_withNotAdmin()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// disable plugins
	bRes = false;
	switchPluginsOnOrOff(bRes, session, psaDisable, true);
	QVERIFY(bRes);

	//////////////////////////////////////////////////////////////////////////
	// try to enable plugins
	if( !TestConfig::isServerMode() )
		QSKIP( "This test is unsupported in not-server mode", SkipAll );
	SimpleServerWrapper sessionNotAdmin( TestConfig::getUserLogin() );

	bRes = false;
	PRL_BOOL bUserCanChangeServerPrefs=PRL_TRUE;
	canUserChangeServerPrefs( bRes, sessionNotAdmin, bUserCanChangeServerPrefs );
	QVERIFY(bRes);

	bRes = false;
	PRL_RESULT retCode = bUserCanChangeServerPrefs
		? PRL_ERR_ENC_ENABLE_PLUGINS_NOT_PERMITTED
		: PRL_ERR_USER_NO_AUTH_TO_EDIT_SERVER_SETTINGS;
	switchPluginsOnOrOff(bRes, sessionNotAdmin, psaEnable, false, retCode );
	QVERIFY(bRes);
}
...
void DispFunctionalityTest::testEncryptionPluginsEnable_authVm()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED

	bool bRes;
	SdkHandleWrap hJob, hVmHandle;
	SimpleServerWrapper session(0);
	AutoRestoreCommonPrefs autoCommonPresRestorer(session);
	...
	// encrypt
	bRes = false;
	createEncryptedVm(bRes, hVmHandle, session, GEN_VM_NAME_BY_TEST_FUNCTION(), eptTestPlugin1 );
	AutoDeleteVm autoDeleter(hVmHandle);
	QVERIFY(bRes);

	//////////////////////////////////////////////////////////////////////////
	// auth from another session
	SimpleServerWrapper session2(0);
	QString sVmUuid = GetVmUuidByHandle( hVmHandle );
	SdkHandleWrap hVmHandle2 = session2.GetVmByUuid( sVmUuid );
	...
	// wrong password
	QString sWrongPswd = DefaultParams::getVmPasswordStr() + "123";
	hJob.reset(PrlVm_Authorise(hVmHandle2, QSTR2UTF8(sWrongPswd), 0 ));
	CHECK_ASYNC_OP_FAILED (hJob, PRL_ERR_WRONG_PASSWORD_TO_ENCRYPTED_VM );

	hJob.reset(PrlVm_Authorise(hVmHandle2, DefaultParams::getVmPassword(), 0 ));
	CHECK_JOB_RET_CODE(hJob);
...
void DispFunctionalityTest::testEncryptionPluginsEnable_LoadTwoPlugins()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	bRes = false;
	doEncryptDecryptPair( bRes, session, GEN_VM_NAME_BY_TEST_FUNCTION(), eptTestPlugin2 );
	QVERIFY(bRes);
}

void DispFunctionalityTest::testEncryptionPluginsEnable_LoadTwoPluginsWithSameId()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// load test plugin from security hole
	LOAD_TEST_PLUGINS_WITH_DUP_ID( session, eptTestPlugin1 );
...
//////////////////////////////////////////////////////////////////////////
void DispFunctionalityTest::testEncryptionPluginsDisable()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	//////////////////////////////////////////////////////////////////////////
	// create encrypted vm / disable plugin / check to decrypt / check to access to vm
	//////////////////////////////////////////////////////////////////////////
	...
	// disable plugins
	bRes = false;
	switchPluginsOnOrOff(bRes, session, psaDisable, false );
	QVERIFY(bRes);

	//////////////////////////////////////////////////////////////////////////
	// check that new password was cleaned
	hJob.reset(PrlVm_GetStatistics(hVmHandle));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM);

	hJob.reset(PrlVm_Authorise(hVmHandle, DefaultParams::getVmPassword(), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_ENC_PLUGIN_UUID_NOT_FOUND);

	hJob.reset(PrlVm_Decrypt(hVmHandle, DefaultParams::getVmPassword(), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM);
}

void DispFunctionalityTest::testEncryptionPluginsDisable_withNotAdmin()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	PRL_RESULT retCode = bUserCanChangeServerPrefs
		...
		: PRL_ERR_USER_NO_AUTH_TO_EDIT_SERVER_SETTINGS;
	switchPluginsOnOrOff(bRes, sessionNotAdmin, psaDisable, false, retCode);
	QVERIFY(bRes);
}

void DispFunctionalityTest::testEncryptionPluginsDisable_withRunningVm()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// try to disable plugins
	bRes = false;
	switchPluginsOnOrOff(bRes, session, psaDisable, false, PRL_ERR_ENC_UNABLE_UNLOAD_PLUGINS_IN_USE );
	QVERIFY(bRes);
...
void DispFunctionalityTest::startSlowEncryptedOperation_andWaitFirstProgressEvent(bool& bRes
	  ...
	  , SdkHandleWrap& hOutJobToCancel )
{
	...
	// create vm  with plain hdd
	// begin to encrypt and wait for first encryption progress event

	// create encrypted vm
	bRes = false;
	quint32 nFlags = DefaultParams::getEncryptionFlags() | cemWithPlainHdd;
	QString sUnused;
	createVmAndPrepareForEncrypt( bRes
		, hServer, sVmName
		, nFlags, hVmHandle, sUnused );
	hOutVmHandle.reset(hVmHandle);
	QVERIFY(bRes);

	startSlowEncryptedOperation_andWaitFirstProgressEvent( bRes, hOutVmHandle, hOutJobToCancel, true );
	QVERIFY(bRes);
}
  ...
  , bool bDoEncrypt )
{
	...
	// 1. check that vm  with plain hdd
	// 2. begin to encrypt and wait for first encryption progress event

	//////////////////////////////////////////////////////////////////////////
	// 1. check that vm  with plain hdd
	QStringList lstPlainHdds;
	GetHddPathsByVmHandle(bRes, hVmHandle, PHD_PLAIN_HARD_DISK, lstPlainHdds );
	QVERIFY(bRes);
	QVERIFY( !lstPlainHdds.isEmpty() );


	//////////////////////////////////////////////////////////////////////////
	// 2. begin to encrypt and wait for first encryption progress event
	SmartPtr<EncrInfoStruct> pEncrInfo( new EncrInfoStruct(bDoEncrypt) );

	QMutexLocker _lock(&pEncrInfo->m_Mutex);

	CHECK_RET_CODE_EXP(PrlVm_RegEventHandler(hVmHandle, EncryptionCallback, pEncrInfo.getImpl()));
	QString sPassword = DefaultParams::getVmPasswordStr();
	QString sPluginId = DefaultParams::getTestPluginUuid();

	if( bDoEncrypt )
		hOutJobToCancel.reset(PrlVm_Encrypt(hVmHandle, QSTR2UTF8(sPassword), QSTR2UTF8(sPluginId), 0));
	else
		hOutJobToCancel.reset(PrlVm_Decrypt(hVmHandle, QSTR2UTF8(sPassword), 0));
	bRes = pEncrInfo->m_Condition.wait(&pEncrInfo->m_Mutex, PRL_JOB_WAIT_TIMEOUT);

	CHECK_RET_CODE_EXP(PrlVm_UnregEventHandler(hVmHandle, EncryptionCallback, pEncrInfo.getImpl())) ;

	QVERIFY(bRes);
}

void DispFunctionalityTest::testEncryptionPluginsDisable_withEncryptOperation()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED

	bool bRes;
	SdkHandleWrap hJob, hVmHandle, hVmHandle2;
	SimpleServerWrapper session(0);
	AutoRestoreCommonPrefs autoCommonPresRestorer(session);
	...
	SdkHandleWrap hJobToCancel;

	// start encrypted op and wait first encryption-progress-changed event
	bRes = false;
	startSlowEncryptedOperation_andWaitFirstProgressEvent( bRes
		, session, GEN_VM_NAME_BY_TEST_FUNCTION()
		, hVmHandle, hJobToCancel );
	AutoDeleteVm autoDeleter(hVmHandle);
	QVERIFY(bRes);
	...
	CHECK_JOB_RET_CODE( PrlJob_Cancel(hJobToCancel) );

	// wait till job finished
	CHECK_ASYNC_OP_FAILED( hJobToCancel, PRL_ERR_OPERATION_WAS_CANCELED );
}

void DispFunctionalityTest::testEncryptionPluginsDisable_tryAuthToVm()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// check that password was cleaned
	hJob.reset(PrlVm_GetStatistics(hVmHandle2));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM);
	...
	//////////////////////////////////////////////////////////////////////////
	// auth from another session
	SimpleServerWrapper session3(0);
	SdkHandleWrap hVmHandle3 = session3.GetVmByUuid( sVmUuid );

	// check fail
	hJob.reset(PrlVm_GetStatistics(hVmHandle3));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM);

	hJob.reset(PrlVm_Authorise(hVmHandle3, DefaultParams::getVmPassword(), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_ENC_PLUGIN_UUID_NOT_FOUND);
}

void DispFunctionalityTest::setDefaultEncryptionPlugin(bool & bRes
													   , const SdkHandleWrap& hServer, const QString& sPluginId
													   , PRL_RESULT nExpectedResult)
{
	bRes = false;
	SdkHandleWrap hDispConfig;
	GET_RESULT_AFTER_ASYNC_CALL( (PrlSrv_GetCommonPrefs(hServer)), hDispConfig );
	...
	CHECK_JOB_RET_CODE( PrlSrv_CommonPrefsBeginEdit(hServer) );
	CHECK_RET_CODE_EXP(	PrlDispCfg_SetDefaultEncryptionPluginId(hDispConfig \
		, QSTR2UTF8(sPluginId)  ) );
	CHECK_ASYNC_OP_FAILED( PrlSrv_CommonPrefsCommitEx(hServer, hDispConfig, 0) \
		, nExpectedResult );

	//////////////////////////////////////////////////////////////////////////
	// do test that value was successfully setted
	if( PRL_SUCCEEDED(nExpectedResult) )
	{
		GET_RESULT_AFTER_ASYNC_CALL( (PrlSrv_GetCommonPrefs(hServer)), hDispConfig );
		QString sValue;
		PRL_EXTRACT_STRING_VALUE( sValue, hDispConfig, PrlDispCfg_GetDefaultEncryptionPluginId );
		QCOMPARE( sValue, sPluginId );
	}

	bRes = true;
}

void DispFunctionalityTest::testEncryptionPluginsDisable_testThatDefaultPluginIsEmpty()
{
	// enable plugins, set test plugin as plugin by default
	// disable plugins, check that default plugin was changed to empty.
	...
	// set test plugin as plugin by default
	bRes = false;
	setDefaultEncryptionPlugin( bRes, session, DefaultParams::getTestPluginUuid() );
	QVERIFY(bRes);
	...
	//////////////////////////////////////////////////////////////////////////
	// do test that value was successfully cleaned
	SdkHandleWrap hDispConfig, hServer = session;
	QString sValue;
	GET_RESULT_AFTER_ASYNC_CALL( (PrlSrv_GetCommonPrefs(hServer)), hDispConfig );
	PRL_EXTRACT_STRING_VALUE( sValue, hDispConfig, PrlDispCfg_GetDefaultEncryptionPluginId );
	QCOMPARE( sValue, QString("") );
}

void DispFunctionalityTest
::testEncryptionPluginsDisable_EncryptVmWithHdds_WithDifferentPluginsId()
{
	// create vm1 with plain hdd encrypted with PARALLELS DEFAULT ALHORITHM
	// create vm2 with plain hdd encrypted with PLUGIN ALGORITHM
	// add vm2.hdd to vm1

	// start slow operation to decrypt vm1
	// by first progress event send "disable plugins"
	// call should be failed by "err-plugin-in-use"
	...
	SdkHandleWrap hSrvConfig;
	GET_SRV_CONFIG( session, hSrvConfig );
	...
	/////////////////////////////////////////////////////////////
	// create vm1 with plain hdd encrypted with PARALLELS DEFAULT ALHORITHM
	QString sVmNameTempl = GEN_VM_NAME_BY_TEST_FUNCTION();
	quint32 nFlags = cemMemFromSnapshots | cemWithPlainHdd;
	bRes = false;
	createEncryptedVm( bRes, hVmHandle, session, sVmNameTempl + "-1", eptDefault, nFlags);
	AutoDeleteVm autoDeleter(hVmHandle);
	QVERIFY(bRes);
	...
	// create vm2 with plain hdd encrypted with PLUGIN ALGORITHM
	bRes = false;
	createEncryptedVm( bRes, hVmHandle2, session, sVmNameTempl + "-2", eptTestPlugin1, cemWithPlainHdd );
	AutoDeleteVm autoDeleter2(hVmHandle2);
	QVERIFY(bRes);

	/////////////////////////////////////////////////////////////
	// add vm2.hdd to vm1
	QList<SdkHandleWrap> lstVm2Hdd;
	GetHddListByVmHandle(bRes, hVmHandle2, lstVm2Hdd, true, PHD_PLAIN_HARD_DISK);
	QVERIFY(bRes);
	QVERIFY( !lstVm2Hdd.isEmpty() );

	SdkHandleWrap hAddedHdd;
	addExistingHddToVm( bRes, hVmHandle, lstVm2Hdd[0], hSrvConfig, hAddedHdd
		, DefaultParams::getVmPasswordStr() );
	QVERIFY(bRes);

	QString sSysName;
	PRL_EXTRACT_STRING_VALUE( sSysName, hAddedHdd, PrlVmDev_GetSysName );
	QString sImageName;
	PRL_EXTRACT_STRING_VALUE( sImageName, hAddedHdd, PrlVmDev_GetImagePath );
	...
	// start DECRYPTION and wait first encryption-progress-changed event
	bRes = false;
	startSlowEncryptedOperation_andWaitFirstProgressEvent( bRes, hVmHandle, hJobToCancel, false );
	QVERIFY(bRes);
...
void DispFunctionalityTest::testSetDefaultEncryptPlugin_testEncryptWithTestPluginByDefault()
{
	// enable plugins, set test plugin as plugin by default
	// disable plugins, check that default plugin was changed to empty.
	...
	//////////////////////////////////////////////////////////////////////////
	// check that new password was hashed
	CHECK_ASYNC_OP_FAILED(PrlVm_GetStatistics(hVmHandle),PRL_ERR_AUTH_REQUIRED_TO_ENCRYPTED_VM);
	CHECK_ASYNC_OP_FAILED(PrlVm_Authorise(hVmHandle, DefaultParams::getVmPassword(), 0) \
		, PRL_ERR_ENC_PLUGIN_UUID_NOT_FOUND);
}

void DispFunctionalityTest::testSetDefaultEncryptPlugin_whenPluginsAreDisabled()
{
	// disable plugins, try to  set test plugin as plugin by default
...
void DispFunctionalityTest::testSetDefaultEncryptPlugin_whenPluginNotFound()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// set test plugin as plugin by default
	bRes = false;
	setDefaultEncryptionPlugin( bRes, session, Uuid::createUuid().toString()
		, PRL_ERR_CANT_SET_DEFAULT_ENCRYPTION_PLUGIN );
	QVERIFY(bRes);
}

void DispFunctionalityTest::testSetDefaultEncryptPlugin_whenPluginIdHasWrongFormat()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// set test plugin as plugin by default
	bRes = false;
	setDefaultEncryptionPlugin( bRes, session, "1282`1893173edoa"
		, PRL_ERR_CANT_SET_DEFAULT_ENCRYPTION_PLUGIN_BY_WRONG_FORMAT );
	QVERIFY(bRes);
}

//////////////////////////////////////////////////////////////////////////
void DispFunctionalityTest::testEncryptionPluginsRescan()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	CHECK_JOB_RET_CODE( PrlSrv_RefreshPlugins(session, 0 ) );
...
void DispFunctionalityTest::testEncryptionPluginsRescan_AddPlugin2AndRescan()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// check that plugin2 wasnt loaded
	bRes = false;
	checkEncryptFailure( bRes, session, GEN_VM_NAME_BY_TEST_FUNCTION(), eptTestPlugin2
		, PRL_ERR_WRONG_CIPHER_ID_FORMAT );
	QVERIFY( bRes );

	// add second plugin
	bRes = false;
	copyPluginToPluginDir( bRes, eptTestPlugin2, getCurrentPluginsDir() );
	QVERIFY(bRes);
...
void DispFunctionalityTest::testEncryptionPluginsRescan_DeletePluginAndRescan()
{
#ifdef _WIN_
	QSKIP( "Unsupported for WINDOWS because test can't remove dll used by another process(disp)"
		, SkipAll );
#endif
	...
	// DELETE PLUGIN AND RESCAN
	// add second plugin
	bRes = false;
	removePluginFromPluginDir( bRes, eptTestPlugin2, getCurrentPluginsDir() );
	QVERIFY(bRes);
...
void DispFunctionalityTest::testEncryptionPluginsRescan_withDisabled()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	CHECK_ASYNC_OP_FAILED( PrlSrv_RefreshPlugins(session, 0 ), PRL_ERR_ENC_PLUGINS_FEATURE_IS_DISABLED );
}

void DispFunctionalityTest::testEncryptionPluginsRescan_withNotAdmin()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	CHECK_ASYNC_OP_FAILED( PrlSrv_RefreshPlugins(sessionNotAdmin, 0 ), PRL_ERR_ENC_RESCAN_PLUGINS_NOT_PERMITTED );

}

void DispFunctionalityTest::testEncryptionPluginsRescan_withRunningVm()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	SmartPtr<AutoStopVm> pAutoStopper( new AutoStopVm(hVmHandle) );

	//////////////////////////////////////////////////////////////////////////
	// try to rescan plugins
	CHECK_ASYNC_OP_FAILED( PrlSrv_RefreshPlugins(session, 0 ), PRL_ERR_ENC_UNABLE_UNLOAD_PLUGINS_IN_USE );

	// stop vm
	pAutoStopper = SmartPtr<AutoStopVm>(0);

	CHECK_JOB_RET_CODE( PrlSrv_RefreshPlugins(session, 0 ) );
}

void DispFunctionalityTest::testEncryptionPluginsRescan_withEncryptOperation()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	// try to disable plugins
	CHECK_ASYNC_OP_FAILED( PrlSrv_RefreshPlugins(session, 0 )
		, PRL_ERR_ENC_UNABLE_UNLOAD_PLUGINS_IN_USE );
...
void DispFunctionalityTest::testEncryptionPluginsRescan_LoadTwoPluginsWithSameId()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	bRes = false;
	copyPluginToPluginDir( bRes, eptTestPlugin2, getCurrentPluginsDir(), true );
	QVERIFY(bRes);
...
void DispFunctionalityTest
	::testEncryptionPluginsRescan_EncryptVmWithHdds_WithDifferentPluginsId()
{
	QSKIP( "Not implemented yet.", SkipAll );
	// FIXME:
	//	test may be implemented through
	//		 testEncryptionPluginsDisable_EncryptVmWithHdds_WithDifferentPluginsId() logic.
...
void DispFunctionalityTest::testEncryptionGetPluginsList()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	//Check twice get plugins list call due https://jira.sw.ru/browse/PDFM-24013
	CHECK_ASYNC_OP_FAILED( PrlSrv_GetPluginsList(session, NULL, 0 ), PRL_ERR_SUCCESS );
	CHECK_ASYNC_OP_FAILED( PrlSrv_GetPluginsList(session, NULL, 0 ), PRL_ERR_SUCCESS );
}

void DispFunctionalityTest::testEncryptionGetPluginsList_SpecialGuiCase()
{
	CHECK_WHETHER_ENCRYPTION_DISABLED
	...
	QString sVmUuid;
	bRes = false;
	createVmAndPrepareForEncrypt( bRes, session, GEN_VM_NAME_BY_TEST_FUNCTION(), PEF_CHECK_PRECONDITIONS_ONLY, m_VmHandle, sVmUuid );
	AutoDeleteVm autoDeleter(m_VmHandle);
	QVERIFY(bRes);

	// Now try to check encrypt preconditions and get plugins list - the same test case as at https://jira.sw.ru/browse/PDFM-24013
	CHECK_ASYNC_OP_FAILED(PrlVm_BeginEdit(m_VmHandle), PRL_ERR_SUCCESS);
	CHECK_ASYNC_OP_FAILED(PrlVm_Encrypt(m_VmHandle, NULL, NULL, PEF_CHECK_PRECONDITIONS_ONLY), PRL_ERR_SUCCESS);
	CHECK_ASYNC_OP_FAILED(PrlSrv_GetPluginsList(session, NULL, 0 ), PRL_ERR_SUCCESS);
}
...
void DispFunctionalityTest::testEncryption_DecryptVmWithHdd_AttachedToAnotherVm_AndChangedPwd()
{
	// 1.1 create vm1, vm2( with plain hdd-1 )
	//		PLAIN HDD is necessary to check operation duration.
	// 1.2 add hdd-2 to vm2
	// 2. encrypt vm1 & vm2 with different passwords
	// 3. attach to vm1 vm2.hdd2 (hdd password was changed)
	// 4. try decrypt vm2
	// 5.1 check that error "Wrong Hdd Password" happens
	// 5.2 check that error returns immediately.
	...
	// 1.1 create vm1
	QString sVmNameTempl = GEN_VM_NAME_BY_TEST_FUNCTION();
	quint32 nFlags = cemMemFromSnapshots;
	bRes = false;
	createEncryptedVm( bRes, hVmHandle, session, sVmNameTempl + "-1", eptDefault, nFlags);
	AutoDeleteVm autoDeleter(hVmHandle);
	QVERIFY(bRes);
	...
	/////////////////////////////////////////////////////////////
	// 1.2 create vm2 with plain hdd
	// PLAIN HDD is necessary to check operation duration.
	nFlags = cemMemFromSnapshots | cemWithPlainHdd;
	bRes = false;
	createEncryptedVm( bRes, hVmHandle2, session, sVmNameTempl + "-2", eptDefault, nFlags);
	AutoDeleteVm autoDeleter2(hVmHandle2);
	QVERIFY(bRes);

	/////////////////////////////////////////////////////////////
	// 1.3 add to vm2 with hdd-2
	QString sPathToWrongHdd;
	{
		bRes = false;
		SdkHandleWrap hHddHandle;
		AddHdd( bRes, hVmHandle2, 64000, PHD_EXPANDING_HARD_DISK, hSrvConfig, hHddHandle);
		QVERIFY(bRes);
		PRL_EXTRACT_STRING_VALUE( sPathToWrongHdd, hHddHandle, PrlVmDev_GetSysName );

		QVERIFY( QFileInfo(sPathToWrongHdd).isAbsolute() );
	}

	//////////////////////////////////////////////////////////////////////////
	// 2. encrypt vm1 & vm2 with different passwords
	//	change password for vm2
	QString sVm2Password =  DefaultParams::getVmPasswordStr() + "123";
	CHECK_JOB_RET_CODE( PrlVm_ChangePassword( hVmHandle2
		, DefaultParams::getVmPassword(), QSTR2UTF8(sVm2Password), 0 ) );
	...
	// add vm2.hdd to vm1
	QList<SdkHandleWrap> lstVm2Hdd;
	GetHddListByVmHandle(bRes, hVmHandle2, lstVm2Hdd, true
		, (PRL_HARD_DISK_INTERNAL_FORMAT)-1, sPathToWrongHdd );
	QVERIFY(bRes);
	QVERIFY( !lstVm2Hdd.isEmpty() );

	SdkHandleWrap hAddedHdd;
	addExistingHddToVm( bRes, hVmHandle, lstVm2Hdd[0], hAddedHdd, hSrvConfig, sVm2Password );
	QVERIFY(bRes);

	// 4. try decrypt vm2
	// 5.1 check that error "Wrong Hdd Password" happens
	// 5.2 check that error returns immediately.
	PRL_UINT64 tsStartTimestamp = PrlGetTickCount64();
	hJob.reset(PrlVm_Decrypt(hVmHandle2, QSTR2UTF8(sVm2Password), 0 ));
	CHECK_ASYNC_OP_FAILED(hJob, PRL_ERR_ENC_WRONG_HDD_PASSWORD );

	double deltaInSecs = (double)( PrlGetTickCount64() - tsStartTimestamp ) / PrlGetTicksPerSecond();
	QVERIFY2( deltaInSecs < 5
		, QSTR2UTF8( QString("operation is too long - %1 secs").arg(deltaInSecs) ) );

}

void DispFunctionalityTest::testEncryption_DecryptVmWithHdd_AttachedToAnotherVm_AndDecrypted()
{
	// check for PRL_ERR_ENC_HDD_IS_UNENCRYPTED
	QSKIP( "Not implemented yet", SkipAll );
}

void DispFunctionalityTest::testEncryption_DecryptVmWithHdd_AttachedToAnotherVm_AndEncryptedWithPlugin2()
{
	// check for PRL_ERR_ENC_HDD_WRONG_ENCRYPTION_ENGINE
	QSKIP( "Not implemented yet", SkipAll );
}

void DispFunctionalityTest::testEncryption_EncryptVmWithHdd_AttachedToAnotherVm_AndEncrypted()
{
	// check for PRL_ERR_ENC_HDD_IS_ALREADY_ENCRYPTED
	QSKIP( "Not implemented yet", SkipAll );
}
