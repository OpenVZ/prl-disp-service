...
void PrlVmDevManipulationsTest::testHardDiskSetPassword()
{
	QString sExpectedPasswd = Uuid::createUuid().toString();

	SdkHandleWrap hVmDev;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev( m_VmHandle, PDE_HARD_DISK, hVmDev.GetHandlePtr() ));
	CHECK_RET_CODE_EXP(PrlVmDevHd_SetPassword( hVmDev, QSTR2UTF8(sExpectedPasswd) ))

	PRL_VOID_PTR pBuffer = NULL;
	CHECK_RET_CODE_EXP(PrlHandle_ToString( hVmDev, &pBuffer ))
	QString sVmHardDisk = UTF8_2QSTR( (const char *)pBuffer );
	PrlBuffer_Free(pBuffer);
	CVmHardDisk _hdd;
	CHECK_RET_CODE_EXP(_hdd.fromString( sVmHardDisk ))
	QCOMPARE(sExpectedPasswd, _hdd.getPassword());
}

void PrlVmDevManipulationsTest::testHardDiskSetEmptyPassword()
{
	SdkHandleWrap hVmDev;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev( m_VmHandle, PDE_HARD_DISK, hVmDev.GetHandlePtr() ));
	CHECK_RET_CODE_EXP(PrlVmDevHd_SetPassword( hVmDev, 0 ))
	...
	CVmHardDisk _hdd;
	CHECK_RET_CODE_EXP(_hdd.fromString( sVmHardDisk ))
	QVERIFY(_hdd.getPassword().isEmpty());
}

void PrlVmDevManipulationsTest::testHardDiskSetPasswordOnWrongParams()
{
	CHECK_CONCRETE_EXPRESSION_RET_CODE(PrlVmDevHd_SetPassword( PRL_INVALID_HANDLE, 0 ), PRL_ERR_INVALID_ARG)
	CHECK_CONCRETE_EXPRESSION_RET_CODE(PrlVmDevHd_SetPassword( m_VmHandle, 0 ), PRL_ERR_INVALID_ARG)
}

void PrlVmDevManipulationsTest::testHardDiskCheckPasswordOnWrongParams()
{
	//Wrong handle case
	CHECK_ASYNC_OP_FAILED(PrlVmDevHd_CheckPassword( PRL_INVALID_HANDLE, 0 ), PRL_ERR_INVALID_ARG)
	CHECK_ASYNC_OP_FAILED(PrlVmDevHd_CheckPassword( m_VmHandle, 0 ), PRL_ERR_INVALID_ARG)

	//Device which not bound to VM case
	SdkHandleWrap hVmDev;
	CHECK_RET_CODE_EXP(PrlVmDev_Create( PDE_HARD_DISK, hVmDev.GetHandlePtr() ));
	CHECK_RET_CODE_EXP(PrlVmDevHd_SetPassword( hVmDev, "" ));
	CHECK_ASYNC_OP_FAILED(PrlVmDevHd_CheckPassword( hVmDev, 0 ), PRL_ERR_INVALID_ARG)
}

void PrlVmDevManipulationsTest::testHardDiskIsEncrypted()
{
	SdkHandleWrap hVmDev;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev( m_VmHandle, PDE_HARD_DISK, hVmDev.GetHandlePtr() ));

	PRL_BOOL bIsEncrypted = PRL_TRUE;
	CHECK_RET_CODE_EXP(PrlVmDevHd_IsEncrypted( hVmDev, &bIsEncrypted ))
	...
	CVmHardDisk _hdd;
	CHECK_RET_CODE_EXP(_hdd.fromString( sVmHardDisk ))
	QCOMPARE(quint32(_hdd.isEncrypted()), quint32(bIsEncrypted));
}

void PrlVmDevManipulationsTest::testHardDiskIsEncryptedOnWrongParams()
{
	//Wrong handle case
	PRL_BOOL bIsEncrypted = PRL_FALSE;
	CHECK_CONCRETE_EXPRESSION_RET_CODE(PrlVmDevHd_IsEncrypted( PRL_INVALID_HANDLE, &bIsEncrypted ), PRL_ERR_INVALID_ARG)
	CHECK_CONCRETE_EXPRESSION_RET_CODE(PrlVmDevHd_IsEncrypted( m_ServerHandle, &bIsEncrypted ), PRL_ERR_INVALID_ARG)

	//Wrong device handle case
	SdkHandleWrap hVmDev;
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev( m_VmHandle, PDE_SOUND_DEVICE, hVmDev.GetHandlePtr() ));
	CHECK_CONCRETE_EXPRESSION_RET_CODE(PrlVmDevHd_IsEncrypted( hVmDev, &bIsEncrypted ), PRL_ERR_INVALID_ARG)

	//Null pointer case
	CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev( m_VmHandle, PDE_HARD_DISK, hVmDev.GetHandlePtr() ));
	CHECK_CONCRETE_EXPRESSION_RET_CODE(PrlVmDevHd_IsEncrypted( hVmDev, 0 ), PRL_ERR_INVALID_ARG)
}
...
void PrlVmDevManipulationsTest::testShadowVmSupportOnTransformPathDevices()
{
	QVERIFY_EXPRESSION(shadowVmSupport_Prepare());

	// VM folder
	QVERIFY_EXPRESSION(shadowVmSupport_CheckRelativeDevPaths());
	QVERIFY_EXPRESSION(shadowVmSupport_CheckAbsoluteDevPaths());

	// Shadow VM folder
	QVERIFY_EXPRESSION(shadowVmSupport_CheckAbsoluteDevPaths_NativeVm());
	QVERIFY_EXPRESSION(shadowVmSupport_CheckRelativeDevPaths_ShadowVm());
	QVERIFY_EXPRESSION(shadowVmSupport_CheckAbsoluteDevPaths_ShadowVm());

	// Out of shadow VM folder
	QVERIFY_EXPRESSION(shadowVmSupport_CheckAbsoluteDevPaths_ExtDir());
}

struct DevParams
{
	PRL_DEVICE_TYPE dt;
	PRL_VM_DEV_EMULATION_TYPE et;
	const char* fname;
};

static DevParams lstDevParams[]=
{
	{PDE_FLOPPY_DISK, PDT_USE_IMAGE_FILE, "floppy"},
	{PDE_HARD_DISK, PDT_USE_IMAGE_FILE, "hard_disk"},
	{PDE_OPTICAL_DISK, PDT_USE_IMAGE_FILE, "optical_disk"},
	{PDE_PARALLEL_PORT, PDT_USE_OUTPUT_FILE, "parallel_port"},
	{PDE_SERIAL_PORT, PDT_USE_OUTPUT_FILE, "serial_port"},
};

void PrlVmDevManipulationsTest::shadowVmSupport_Prepare()
{
// Prepare temporary folders

	m_qsTestPath = QDir::tempPath() + "/" + Uuid::createUuid().toString();
	QVERIFY(QDir().mkpath(m_qsTestPath));
	QString qsVmShadowDir = m_qsTestPath;
	QVERIFY(QFile::setPermissions(m_qsTestPath, QFile::permissions(m_qsTestPath) | QFile::WriteOther));

	m_qsTestPath2 = QDir::tempPath() + "/" + Uuid::createUuid().toString();
	QVERIFY(QDir().mkpath(m_qsTestPath2));

// Prepare VM without devices

	CHECK_RET_CODE_EXP(PrlVmCfg_SetDefaultConfig(
						m_VmHandle, PRL_INVALID_HANDLE, PVS_GUEST_VER_WIN_WINDOWS7, PRL_FALSE));
	CHECK_RET_CODE_EXP(PrlVmCfg_SetName(m_VmHandle, QTest::currentTestFunction()));

	SdkHandleWrap hJob(PrlVm_Reg(m_VmHandle, QSTR2UTF8(qsVmShadowDir), PRL_TRUE));
	CHECK_JOB_RET_CODE(hJob);

// Prepare devices inside VM

	hJob.reset(PrlVm_BeginEdit(m_VmHandle));
	CHECK_JOB_RET_CODE(hJob);

	QString qsVmConfigPath;
	PRL_EXTRACT_STRING_VALUE(qsVmConfigPath, m_VmHandle, PrlVmCfg_GetHomePath);
	QString qsVmDir = QFileInfo(qsVmConfigPath).path();

	for(size_t i = 0; i < sizeof(lstDevParams)/sizeof(lstDevParams[0]); i++)
	{
		DevParams dp = lstDevParams[i];

		QString qsDevPath = (qsVmDir + "/") + dp.fname;
		{
			QFile file(qsDevPath);
			QVERIFY(file.open(QIODevice::WriteOnly));
			QVERIFY(file.write(QByteArray("some content")) != 0);
		}

		SdkHandleWrap hDev;
		CHECK_RET_CODE_EXP(PrlVmCfg_CreateVmDev(m_VmHandle, dp.dt, hDev.GetHandlePtr()));

		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDev, QSTR2UTF8(qsDevPath) ));
		CHECK_RET_CODE_EXP(PrlVmDev_SetFriendlyName(hDev, QSTR2UTF8(qsDevPath) ));
		CHECK_RET_CODE_EXP(PrlVmDev_SetEmulatedType(hDev, dp.et));
	}
...
void PrlVmDevManipulationsTest::shadowVmSupport_CheckRelativeDevPaths()
{
	QString qsVmConfigPath;
	PRL_EXTRACT_STRING_VALUE(qsVmConfigPath, m_VmHandle, PrlVmCfg_GetHomePath);
	...
	CVmConfiguration vm_config;
	{
		QFile file(qsVmConfigPath);
		CHECK_RET_CODE_EXP(vm_config.loadFromFile(&file, false));
	}

	CHECK_RET_CODE_EXP(PrlHandle_FromString(m_VmHandle, QSTR2UTF8(vm_config.toString()) ));
	...
	for(size_t i = 0; i < sizeof(lstDevParams)/sizeof(lstDevParams[0]); i++)
	{
		...
		SdkHandleWrap hDev;
		CHECK_RET_CODE_EXP(PrlVmCfg_GetDevByType(m_VmHandle, dp.dt, 0, hDev.GetHandlePtr()));

		QString qsPath;
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetSysName);
		QCOMPARE(qsPath, QString(dp.fname));
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetFriendlyName);
		QCOMPARE(qsPath, QString(dp.fname));
	}
}

void PrlVmDevManipulationsTest::shadowVmSupport_CheckAbsoluteDevPaths()
{
	QString qsVmShadowDir = m_qsTestPath;
	...
	for(size_t i = 0; i < sizeof(lstDevParams)/sizeof(lstDevParams[0]); i++)
	{
		...
		QString qsPath;
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetSysName);
		QCOMPARE(qsPath,  (qsVmDir + "/") + dp.fname);
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetFriendlyName);
		QCOMPARE(qsPath,  (qsVmDir + "/") + dp.fname);

		// Move devices files up to VM native folder
		QString qsNewPath = (qsVmShadowDir + "/") + dp.fname;
		QVERIFY(QFile::rename(qsPath, qsNewPath));
		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDev, QSTR2UTF8(qsNewPath) ));
		CHECK_RET_CODE_EXP(PrlVmDev_SetFriendlyName(hDev, QSTR2UTF8(qsNewPath) ));
	}
...
void PrlVmDevManipulationsTest::shadowVmSupport_CheckAbsoluteDevPaths_NativeVm()
{
	QString qsVmShadowDir = m_qsTestPath;
	...
	for(size_t i = 0; i < sizeof(lstDevParams)/sizeof(lstDevParams[0]); i++)
	{
		...
		QString qsPath;
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetSysName);
		QCOMPARE(qsPath,  (qsVmShadowDir + "/") + dp.fname);
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetFriendlyName);
		QCOMPARE(qsPath,  (qsVmShadowDir + "/") + dp.fname);
	}
}

void PrlVmDevManipulationsTest::shadowVmSupport_CheckRelativeDevPaths_ShadowVm()
{
	SdkHandleWrap hJob(PrlVm_BeginEdit(m_VmHandle));
	CHECK_JOB_RET_CODE(hJob);
	...
	CVmConfiguration vm_config;
	PRL_VOID_PTR pVmConfig = 0;
	CHECK_RET_CODE_EXP(PrlHandle_ToString(m_VmHandle, &pVmConfig));
	CHECK_RET_CODE_EXP(vm_config.fromString( UTF8_2QSTR((const char * )pVmConfig) ));
	PrlBuffer_Free(pVmConfig);

	vm_config.getExternalConfigInfo()->setType(PEVT_VMWARE);
	...
	{
		QFile file(vm_config.getVmIdentification()->getHomePath());
		CHECK_RET_CODE_EXP(vm_config.loadFromFile(&file, false));
	}
...
void PrlVmDevManipulationsTest::shadowVmSupport_CheckAbsoluteDevPaths_ShadowVm()
{
	QString qsVmShadowDir = m_qsTestPath;
	QString qsExtDir = m_qsTestPath2;
	...
	for(size_t i = 0; i < sizeof(lstDevParams)/sizeof(lstDevParams[0]); i++)
	{
		...
		// Move devices files to external folder
		QString qsNewPath = (qsExtDir + "/") + dp.fname;
		QVERIFY(QFile::rename(qsPath, qsNewPath));
		CHECK_RET_CODE_EXP(PrlVmDev_SetSysName(hDev, QSTR2UTF8(qsNewPath) ));
		CHECK_RET_CODE_EXP(PrlVmDev_SetFriendlyName(hDev, QSTR2UTF8(qsNewPath) ));
	}
...
void PrlVmDevManipulationsTest::shadowVmSupport_CheckAbsoluteDevPaths_ExtDir()
{
	QString qsExtDir = m_qsTestPath2;
	...
	for(size_t i = 0; i < sizeof(lstDevParams)/sizeof(lstDevParams[0]); i++)
	{
		...
		QString qsPath;
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetSysName);
		QCOMPARE(qsPath,  (qsExtDir + "/") + dp.fname);
		PRL_EXTRACT_STRING_VALUE(qsPath, hDev,  PrlVmDev_GetFriendlyName);
		QCOMPARE(qsPath,  (qsExtDir + "/") + dp.fname);
	}
}
