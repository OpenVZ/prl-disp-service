...
void Server::onTimer ()
{
    ...
    foreach ( IOSender::Handle h, clients ) {
        IOSender::Statistics cliStat;
        bool res = m_server->connectionStatistics( h, cliStat );
        Q_ASSERT(res);
        Q_UNUSED(res);
        stat.receivedPackages += cliStat.receivedPackages;
        stat.sentPackages += cliStat.sentPackages;
        stat.receivedBytes += cliStat.receivedBytes;
        stat.sentBytes += cliStat.sentBytes;
    }
